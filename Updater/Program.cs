...
namespace Net
{
    public static class Updater
    {
        ...
        public static string GetCurrentRepo()
        {
            try
            {
                var temp = File.ReadAllLines(AppPath + "\\Canary\\Branch")[1];
                if (temp == null || temp == "") throw new ArgumentNullException();
        ...
        public static string GetCurrentBranch()
        {
            try
            {
                var temp = File.ReadAllLines(AppPath + "\\Canary\\Branch")[0];
                if (temp == null || temp == "") throw new ArgumentNullException();
        ...
        public static async Task CheckUpdate(bool Overwrite, string releaseTag = "", bool manual = false,
            string openFile = null, bool checkDocumentation = false, bool Automatic = false)
        {
            // If canary is active, disable it
            if (File.Exists(AppPath + "\\Canary\\Active")) File.Delete(AppPath + "\\Canary\\Active");

            var repoOwner = mainRepo.Split('/')[0];
            var repoName = mainRepo.Split('/')[1];
            ...
            try
            {
                ...
                try
                {
                    using (var s = new Ping())
                    {
                        var status = s.Send("www.github.com").Status;
                        Console.WriteLine(status);
                        if (status != IPStatus.Success)
                        {
                            Console.WriteLine("Failed to connect");
                            if (manual) MessageBox.Show("Unable to connect to GitHub. The website may be down.");
                            return;
                        }
                    }
                }
                ...
                // get Release
                var AllReleases = await github.Repository.Release.GetAll(repoOwner, repoName);
                IReadOnlyList<Release> releases = null;
                Release release = null;
                var documentation = false;
                if (AllReleases.Count == 0) goto UpdateDL;
                ...
                {
                    if (manual) MessageBox.Show("No updates found.");
                    return;
                }
                ...
                if (Automatic)
                {
                    ...
                    if (GetOpenWindowsCount() > 1 &&
                        MessageBox.Show(
                            ...
                            "Updater", MessageBoxButtons.YesNo) != DialogResult.Yes) return;
                }
            ...
            {
                if (manual) MessageBox.Show("Unable to connect to the internet.");
            }
            catch (Exception e)
            {
                if (manual) MessageBox.Show(e.Message);
            }
        }
        ...
        public static async Task CheckCanaryUpdate(string openFile = null, bool manual = false, bool force = false)
        {
            ...
            {
                char[] slashes = {'\\', '/'};
                var repoData = currentRepo.Split(slashes);
                var release = await github.Repository.Release.Get(repoData[0], repoData[1],
                    "Canary" + (currentBranch.Equals(mainBranch, StringComparison.OrdinalIgnoreCase)
                        ? ""
                        : "-" + currentBranch));
                if (!force)
                {
                    var oldID = File.ReadAllLines(AppPath + "\\Canary\\New")[2];
                    var newID = release.TargetCommitish;
                ...
                if (release == null || release.Assets.Count == 0) throw new Exception();
                if ((manual || GetOpenWindowsCount() > 1) && MessageBox.Show(
                        "Update to #" + release.TargetCommitish +
                        " was found. Would you like to download now?\n\nAll current windows will be closed and changes will be lost!",
                        "Canary Updater", MessageBoxButtons.YesNo) != DialogResult.Yes) return;
            ...
            bool Documentation, string openFile)
        {
            try
            {
                ...
                if (Overwrite && !Documentation) await KillOpenWindows();
                    ...
                    new FileInfo(AppPath + "/temp.exe").Length == 0)
                {
                    ...
                    if (File.Exists(AppPath + "/temp.exe")) File.Delete(AppPath + "/temp.exe");
                ...
                if (Process.GetProcessesByName("winlogon").Count() == 0 || Documentation || !Overwrite)
                {
                    try
                    {
                        var update = Process.Start(AppPath + "/temp.exe", "-o\"" + AppPath + "\" -y");
                        // For documentation updates, ensure temp.exe is properly deleted and show changelog if the download was automated.
                        if (Documentation)
                        {
                            update.WaitForExit();
                            if (File.Exists(AppPath + "\\temp.exe")) File.Delete(AppPath + "\\temp.exe");
                ...
                var updateBat = Process.Start(new ProcessStartInfo
                {
        ...
        public static async Task ForceDownloadDocumentation()
        {
            ...
            if (release != null) await DownloadRelease(release, true, true, false, true, "<null>");
        }
        ...
        public static async Task ForceDownloadStable(string openFile = null)
        {
            ...
            IReadOnlyList<Release> releases = (await github.Repository.Release.GetAll(repoOwner, repoName))
                .Where(r => !r.Prerelease).ToList();
            if (releases.Count > 0) await DownloadRelease(releases[0], true, true, false, false, openFile);
        }
        ...
        public static void WriteBatchScript(string openFile)
        {
            using (var sw = new StreamWriter(AppPath + "/Update.bat"))
            {
                ...
                if (File.Exists(AppPath + "/BrawlCrate.exe")) sw.WriteLine("del BrawlCrate.exe /s /f /q");
                if (File.Exists(AppPath + "/BrawlBox.exe")) sw.WriteLine("del BrawlBox.exe /s /f /q");
                if (File.Exists(AppPath + "/BrawlScape.exe")) sw.WriteLine("del BrawlScape.exe /s /f /q");
                if (File.Exists(AppPath + "/SmashBox.exe")) sw.WriteLine("del SmashBox.exe /s /f /q");
                if (File.Exists(AppPath + "/StageBox.exe")) sw.WriteLine("del StageBox.exe /s /f /q");
                if (File.Exists(AppPath + "/color_smash.exe")) sw.WriteLine("del color_smash.exe /s /f /q");
                if (File.Exists(AppPath + "/sawndz.exe")) sw.WriteLine("del sawndz.exe /s /f /q");
                if (File.Exists(AppPath + "/Updater.exe")) sw.WriteLine("del sawndz.exe /s /f /q");

                // Delete DLLs where found/applicable
                if (File.Exists(AppPath + "/BrawlLib.dll")) sw.WriteLine("del BrawlLib.dll /s /f /q");
                if (File.Exists(AppPath + "/Octokit.dll")) sw.WriteLine("del Octokit.dll /s /f /q");
                if (File.Exists(AppPath + "/OpenTK.dll")) sw.WriteLine("del OpenTK.dll /s /f /q");
                if (File.Exists(AppPath + "/discord-rpc.dll")) sw.WriteLine("del discord-rpc.dll /s /f /q");
                if (File.Exists(AppPath + "/IronPython.dll")) sw.WriteLine("del IronPython.dll /s /f /q");
                if (File.Exists(AppPath + "/IronPython.Modules.dll"))
                    sw.WriteLine("del IronPython.Modules.dll /s /f /q");
                if (File.Exists(AppPath + "/IronPython.SQLite.dll")) sw.WriteLine("del IronPython.SQLite.dll /s /f /q");
                if (File.Exists(AppPath + "/IronPython.Wpf.dll")) sw.WriteLine("del OpenTK.dll /s /f /q");
                if (File.Exists(AppPath + "/IronPython.Wpf.dll")) sw.WriteLine("del IronPython.Wpf.dll /s /f /q");
                if (File.Exists(AppPath + "/Microsoft.Dynamic.dll")) sw.WriteLine("del Microsoft.Dynamic.dll /s /f /q");
                ...
                if (openFile != null && openFile != "<null>") sw.Write(" \"" + openFile + "\"");
        ...
        public static async Task WriteCanaryTime(string commitid = null, string branchName = null, string repo = null)
        {
            try
            {
                if (commitid != null) Console.WriteLine("Attempting to set Canary using sha: " + commitid);

                branchName = branchName ?? mainBranch;
                repo = repo ?? mainRepo;

                var CanaryDir = Directory.CreateDirectory(AppPath + "\\Canary");
                ...
                var Filename = AppPath + "\\Canary\\New";
        ...
        public static async Task SetCanaryActive()
        {
            var CanaryDir = Directory.CreateDirectory(AppPath + "\\Canary");
            CanaryDir.Attributes = FileAttributes.Directory | FileAttributes.Hidden;
            if (!File.Exists(AppPath + "\\Canary\\Active")) File.Create(AppPath + "\\Canary\\Active");
        ...
        public static async Task ShowCanaryChangelog()
        {
            ...
            string newRepo;
            string oldRepo;
            var Filename = AppPath + "\\Canary\\Old";
            try
            {
            ...
            try
            {
                var cr = new Credentials(Encoding.Default.GetString(_rawData));
                var github = new GitHubClient(new ProductHeaderValue("BrawlCrate")) {Credentials = cr};
                Branch branch;
                try
                {
                ...
                var commits = (await github.Repository.Commit.GetAll(repoData[0], repoData[1], options)).ToList();
                var i = -1;
                ...
                for (var j = i; j >= 0; j--)
                {
                    ...
                    try
                    {
                        var s = "#" + c.Sha.Substring(0, 7) + "@" + currentRepo + '\\' + currentBranch + " by " +
                                c.Author.Login + "\n";
                        changelog += s;
                    }
                    catch
                ...
                var logWindow = new CanaryChangelogViewer(newSha.Substring(0, 7), changelog);
        ...
        public static async Task KillOpenWindows()
        {
            ...
            TRY_AGAIN:
            var px = Process.GetProcessesByName("BrawlCrate");
            var pToClose = px.Where(x => x.MainModule.FileName.Equals(AppPath + "\\BrawlCrate.exe")).ToArray();
            var p = px.FirstOrDefault(x => x.MainModule.FileName.Equals(AppPath + "\\BrawlCrate.exe"));
            if (p != null && p != default(Process) && px != null && pToClose != null && pToClose.Length > 1)
            {
                try
                {
                    foreach (var pNext in pToClose) pNext.Kill();
            ...
            if (p != null && p != default(Process)) p.Kill();
        }

        public static int GetOpenWindowsCount()
        {
            var px = Process.GetProcessesByName("BrawlCrate");
            var pToFind = px.Where(x => x.MainModule.FileName.Equals(AppPath + "\\BrawlCrate.exe")).ToArray();
            ...
            try
            {
                ...
                if (!TagName.StartsWith("BrawlCrate Canary", StringComparison.OrdinalIgnoreCase))
                {
                    ...
                    if (releases != null && releases.Count > 0 && releases[0].TagName != TagName)
                    {
                        ...
                        {
                            var OverwriteResult = MessageBox.Show("Overwrite current installation?", "",
                    ...
                    foreach (var i in issues)
                        if (i.State == ItemState.Open)
                        {
                                ...
                                desc.Contains(TagName))
                            {
                                ...
                                var x = await github.Issue.Update("BrawlCrate", "BrawlCrateIssues", i.Number, update);
                            }
                        }
                ...
                if (!found)
                {
                    ...
                    var x = await github.Issue.Create("BrawlCrate", "BrawlCrateIssues", issue);
                }
            }
            catch
            {
        ...
        private static void Main(string[] args)
        {
            ...
            if (args.Length > 0)
            {
                switch (args[0])
                {
                    case "-r": //overwrite
                        somethingDone = true;
                        var t = Updater.CheckUpdate(true);
                        t.Wait();
                        break;
                    case "-n": // Update in new folder
                        somethingDone = true;
                        var t1 = Updater.CheckUpdate(false);
                        t1.Wait();
                        break;
                    case "-bu": //BrawlCrate update call
                        somethingDone = true;
                        var t2 = Updater.CheckUpdate(args[1] != "0", args[2], args[3] != "0", args[4], args[5] != "0",
                            args[6] != "0");
                        t2.Wait();
                        break;
                    case "-buc": //BrawlCrate Canary update call
                        somethingDone = true;
                        var t2c = Updater.CheckCanaryUpdate(args[1], args[2] != "0");
                        t2c.Wait();
                        break;
                    case "-bi": //BrawlCrate issue call
                        somethingDone = true;
                        var t3 = BugSquish.CreateIssue(args[1], args[2], args[3], args[4], args[5]);
                        t3.Wait();
                        break;
                    case "-bcommitTime": //Called on build to ensure time is saved
                        ...
                        if (args.Length > 1)
                        {
                            ...
                            {
                                t4arg2 = args[2];
                                if (args.Length > 3) t4arg3 = args[3];
                            }
                        }

                        var t4 = Updater.WriteCanaryTime(t4arg1, t4arg2, t4arg3);
                        t4.Wait();
                        break;
                    case "-dlCanary": // Force download the latest Canary build
                        somethingDone = true;
                        var t5 = Updater.CheckCanaryUpdate(args[1], false, true);
                        t5.Wait();
                        break;
                    case "-dlStable": // Force download the latest Stable build
                        somethingDone = true;
                        var t6 = Updater.ForceDownloadStable(args[1]);
                        t6.Wait();
                        break;
                    case "-dlDoc": // Force download the latest Documentation build
                        somethingDone = true;
                        var t6d = Updater.ForceDownloadDocumentation();
                        t6d.Wait();
                        break;
                    case "-canarylog": // Show changelog for canary
                        somethingDone = true;
                        var t7 = Updater.ShowCanaryChangelog();
                        t7.Wait();
                        break;
                    case "-canaryOn": // Activate canary build
                        somethingDone = true;
                        var t8 = Updater.SetCanaryActive();
                        t8.Wait();
                        break;
                    case "-canaryOff":
                        somethingDone = true;
                        var t9 = Updater.SetCanaryInactive();
                        t9.Wait();
                        break;
                    case "-killAll":
                        somethingDone = true;
                        var t10 = Updater.KillOpenWindows();
                        t10.Wait();
                        break;
                }
            }
            ...
            if (!somethingDone) Console.WriteLine(Usage);
        }
    }
}
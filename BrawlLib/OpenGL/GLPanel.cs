...
namespace BrawlLib.OpenGL
{
    ...
    public abstract class GLPanel : UserControl, IEnumerable<GLViewport>
    {
        ...
        public static GLPanel Current
        {
            get
            {
                if (_currentPanel == null && TKContext.CurrentContext != null) TKContext.CurrentContext.Capture(true);
        ...
        public GLViewport CurrentViewport
        {
            get
            {
                ...
                {
                    if (_viewports.Count == 0) CreateDefaultViewport();

                    _currentViewport = _viewports[0];
                }

                if (_currentViewport._owner != this) _currentViewport._owner = this;
            ...
            set
            {
                ...
                if (!_viewports.Contains(_currentViewport) && _currentViewport != null) AddViewport(_currentViewport);
            }
        }
        ...
        {
            if (index < 0 || index >= _viewports.Count) return;

            var v = _viewports[index];
            v._owner = null;
            v.OnInvalidate -= Invalidate;
        ...
        public void FitViewports()
        {
            ...
            foreach (var v in _viewports)
            {
                ...
                var diff = new Vector4(float.MaxValue);
                foreach (var v2 in _viewports)
                {
                    if (v2 == v) continue;

                    var diff2 = Math.Abs(p._x - v2.Percentages._x);
                    if (diff2 <= diff._x)
                    {
                        x = v2;
                        diff._x = diff2;
                ...
                var average = new Vector4(0.0f, 0.0f, 1.0f, 1.0f);
        ...
        {
            if ((_updateCounter = Math.Max(_updateCounter - 1, 0)) == 0) Invalidate();
        }
        ...
        public new void Capture()
        {
            ...
            if (_ctx == null) _ctx = new TKContext(this);
        ...
        protected virtual void OnContextChanged(bool isNowCurrent)
        {
            ...
            if (isNowCurrent) OnResize(EventArgs.Empty);
        ...
        {
            if (_viewports.Count == 1) return _viewports[0];

            x = x.Clamp(0, Width);
            y = Height - y.Clamp(0, Height);

            foreach (var w in _viewports)
                if (x >= w.Region.X &&
        ...
        {
            float val = 0;
            GL.ReadPixels(x, Height - y, 1, 1, PixelFormat.DepthComponent, PixelType.Float, ref val);
            return val;
        }
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            if (_ctx == null)
                ...
                try
                {
                    ...
                    if (code != ErrorCode.NoError) this.Reset(); //Stops the red X of death in its tracks
                }
                finally
                {
                    Monitor.Exit(_ctx);
        ...
        {
            if (_viewports.Count > 1) _highlightedViewport = GetViewport(e.X, e.Y);
        ...
        {
            var camera = CurrentViewport.Camera;
            ...
            var angleX = Math.IEEERemainder(-camera._rotation._y, 180.0);
            if (angleX < -90.0f) angleX = -180.0f - angleX;

            if (angleX > 90.0f) angleX = 180.0f - angleX;

            var angleY = Math.IEEERemainder(camera._rotation._x, 180.0);
            if (angleY < -90.0f) angleY = -180.0f - angleY;

            if (angleY > 90.0f) angleY = 180.0f - angleY;

            var lenX = (float) (Math.Tan(angleX * Math.PI / 180.0) * a);
            var lenY = (float) (Math.Tan(angleY * Math.PI / 180.0) * a);
        ...
        protected override void OnResize(EventArgs e)
        {
            ...
            foreach (var v in _viewports) v.Resize();
        ...
        protected virtual void OnRender(PaintEventArgs e)
        {
            ...
            foreach (var v in _viewports) OnRenderViewport(e, v);

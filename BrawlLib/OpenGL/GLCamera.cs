...
namespace BrawlLib.OpenGL
{
    public unsafe class GLCamera
    {
        ...
        public void Rotate(Vector3 v)
        {
            ...
            if (_rotation._x < -90.0f || _rotation._x > 90.0f) v._y = -v._y;
        ...
        {
            if (!_updating && OnPositionChanged != null) OnPositionChanged();
        }
        ...
        public Vector3 Project(Vector3 source)
        {
            ...
            //Also the order of the matrix multiplication matters
            var t1 = _matrix * (Vector4) source;
            var t2 = _projectionMatrix * t1;
        ...
        {
            //Get ray points
            var ray1 = UnProject(screenPoint._x, screenPoint._y, 0.0f);
            var ray2 = UnProject(screenPoint._x, screenPoint._y, 1.0f);

            if (!Maths.LineSphereIntersect(ray1, ray2, center, radius, out var point))
            {
                ...
                if (clamp) point = Maths.PointAtLineDistance(center, point, radius);
            }
        ...
        {
            if (!_ortho) _rotation = new Vector3();

            var position = point + new Vector3(0.0f, 0.0f, distance);

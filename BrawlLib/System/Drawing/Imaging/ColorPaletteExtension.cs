...
namespace System.Drawing.Imaging
{
    ...
    public static class ColorPaletteExtension
    {
        ...
        {
            var pal = (ColorPalette) typeof(ColorPalette).InvokeMember("",
        ...
        public static void Clamp(this ColorPalette pal, WiiPaletteFormat format)
        {
            switch (format)
            {
                ...
                {
                    for (var i = 0; i < pal.Entries.Length; i++) pal.Entries[i] = (Color) (IA8Pixel) pal.Entries[i];
                ...
                {
                    for (var i = 0; i < pal.Entries.Length; i++) pal.Entries[i] = (Color) (wRGB565Pixel) pal.Entries[i];
                ...
                {
                    for (var i = 0; i < pal.Entries.Length; i++) pal.Entries[i] = (Color) (wRGB5A3Pixel) pal.Entries[i];
        ...
        public static int FindMatch(this ColorPalette pal, ARGBPixel pixel)
        {
            ...
            {
                var dist = pixel.DistanceTo(pal.Entries[i]);
                if (dist < bestDist)
                {
                    bestIndex = i;
                    if (dist == 0) break;

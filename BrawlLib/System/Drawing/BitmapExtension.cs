...
namespace System
{
    public static class BitmapExtension
    {
        ...
        public static Bitmap Copy(this Bitmap sourceImage)
        {
            ...
            {
                //Non-indexed
                var targetImage = new Bitmap(sourceImage.Width, sourceImage.Height, sourceImage.PixelFormat);
        ...
        {
            var colors = new HashSet<ARGBPixel>();
        ...
        public static unsafe Bitmap IndexColors(this Bitmap src, ColorPalette palette, PixelFormat format)
        {
            ...
            var entries = palette.Entries.Length;
            switch (format)
            {
            ...
            var dData = dst.LockBits(new Rectangle(0, 0, w, h), ImageLockMode.ReadWrite, format);

            var sPtr = (ARGBPixel*) sData.Scan0;
            for (var y = 0; y < h; y++)
            {
                var dPtr = (byte*) dData.Scan0 + y * dData.Stride;
                for (var x = 0; x < w; x++)
                {
                    var p = *sPtr++;

                    int bestDist = int.MaxValue, bestIndex = 0;
                    for (var z = 0; z < entries; z++)
                    {
                        var dist = p.DistanceTo(palette.Entries[z]);
                        if (dist < bestDist)
                        {
                            bestDist = dist;
                            bestIndex = z;
        ...
        public static unsafe void Clamp(this Bitmap bmp, WiiPixelFormat format)
        {
            ...
            for (ARGBPixel* ptr = (ARGBPixel*) data.Scan0, ceil = ptr + w * h; ptr < ceil;) clmp(ptr++);
        ...
        {
            var w = Math.Min(bmp.Width, dest.Width);
            var h = Math.Min(bmp.Height, dest.Height);
            using (var dib = new DIB(w, h, dest.PixelFormat))
            {
        ...
        public static unsafe Bitmap Clone(this Bitmap src, int width, int height, int skip)
        {
            ...
            var srcData = src.LockBits(new Rectangle(0, 0, sw, sh), ImageLockMode.ReadOnly, format);
            var dstData = dst.LockBits(new Rectangle(0, 0, width, height), ImageLockMode.ReadWrite, format);
            ...
            switch (format)
            {
                case PixelFormat.Format4bppIndexed:
                {
                    ...
                    {
                        var sPtr = (byte*) srcData.Scan0 + sy * srcData.Stride;
                        var dPtr = (byte*) dstData.Scan0 + dy * dstData.Stride;
                        for (int sx = skip / 2, dx = 0; sx < sw && dx < width; dx++, sx += skip)
                        {
                            var value = sx % 2 == 0 ? (byte) (sPtr[sx >> 1] >> 4) : (byte) (sPtr[sx >> 1] & 0x0F);
                            dPtr[dx >> 1] = dx % 2 == 0
        ...
        {
            if (level <= 1) return (Bitmap) bmp.Clone();

            var scale = 1 << (level - 1);
            int w = bmp.Width / scale, h = bmp.Height / scale;

            var dst = new Bitmap(w, h, bmp.PixelFormat);

            //Step-scale indexed elements
            if (bmp.IsIndexed())
            {
                var srcData = bmp.LockBits(new Rectangle(0, 0, bmp.Width, bmp.Height), ImageLockMode.ReadOnly,
                    bmp.PixelFormat);
                var dstData = dst.LockBits(new Rectangle(0, 0, w, h), ImageLockMode.ReadWrite, bmp.PixelFormat);

                var xStep = (float) bmp.Width / w;
                var yStep = (float) bmp.Height / h;
                int x, y;
                float fx, fy;
                ...
                if (bmp.PixelFormat == PixelFormat.Format8bppIndexed)
                    for (y = 0, fy = 0.5f; y < h; y++, fy += yStep, dPtr += dstData.Stride)
                    {
                        ...
                        for (x = 0, fx = 0.5f; x < w; x++, fx += xStep) dPtr[x] = sPtr[(int) fx];
                    }
                else
                    for (y = 0, fy = 0.5f; y < h; y++, fy += yStep, dPtr += dstData.Stride)
                    {
        ...
        public static unsafe Bitmap InvertColors(this Bitmap bmp)
        {
            if (bmp.PixelFormat == PixelFormat.Format32bppArgb)
            {
                ...
                var inData = bmp.LockBits(rect, ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);
                var outData = output.LockBits(rect, ImageLockMode.ReadOnly, PixelFormat.Format32bppArgb);

                var inPtr = (uint*) inData.Scan0;
                var outPtr = (uint*) outData.Scan0;

                var length = Math.Abs(inData.Stride) * bmp.Height / sizeof(uint);

                for (var i = 0; i < length; i++) outPtr[i] = inPtr[i] ^ 0xFFFFFF;
        ...
        public static unsafe Bitmap InvertAlpha(this Bitmap bmp)
        {
            if (bmp.PixelFormat == PixelFormat.Format32bppArgb)
            {
                ...
                for (var i = 0; i < length; i++) outPtr[i] = inPtr[i] ^ 0xFF000000;
        ...
        public static unsafe Bitmap SwapAlphaAndRGB(this Bitmap bmp)
        {
            if (bmp.PixelFormat == PixelFormat.Format32bppArgb)
            {
                ...
                try
                {
                    ...
                    {
                        var A = (byte) (inPtr[i] >> 24);
                        var R = (byte) (inPtr[i] >> 16);
                        var G = (byte) (inPtr[i] >> 8);
                        var B = (byte) inPtr[i];
                        if (R != G || G != B)
        ...
        /// </summary>
        /// <returns>
        ///     true if the number of distinct pixel values in the
        ///     fully transparent section of the image is more than the number
        ///     elsewhere (including alpha values); false otherwise.
        /// </returns>
        public static unsafe bool GuessIfAlphaInverted(this Bitmap bmp)
        {
            ...
            if (bmp.PixelFormat == PixelFormat.Format32bppArgb)
            {
                ...
                Marshal.Copy((IntPtr) inPtr, pixels, 0, length);
            ...
            var colorsInTransparentSection = new HashSet<int>();
            var colorsInNonTransparenSection = new HashSet<int>();

            foreach (var pixel in pixels)
            {
                var alpha = (byte) ((pixel & 0xFF000000) >> 24);
                if (alpha == 0)
    ...
    {
        public ARGBPixel[] UniqueColors { get; }

        public int ColorCount => UniqueColors.Length;
        public int AlphaColors { get; }
        ...
        public ColorInformation(ARGBPixel[] colors)
        {
            ...
            {
                if (p.A != 0xFF) AlphaColors++;

                if (IsGreyscale && !p.IsGreyscale()) IsGreyscale = false;
            }
        }
    }

...
namespace System
{
    ...
    public unsafe struct Vector4 : ISerializable
    {
        ...
        public static Vector4 operator *(Vector4 v1, Vector4 v2)
        {
            ...
            var x1 = new Vector3(v1._x, v1._y, v1._z);
            var x2 = new Vector3(v2._x, v2._y, v2._z);
            var w1 = v1._w;
            var w2 = v2._w;
        ...
        {
            if (obj is Vector4) return this == (Vector4) obj;
        ...
        {
            var q = this;
            if (q._w > 1.0f) q.Normalize();

            var result = new Vector4
            {
                _w = 2.0f * (float) Math.Acos(q._w)
            };
            var den = (float) Math.Sqrt(1.0 - q._w * q._w);
            if (den > 0.0001f)
            {
                result._x = q._x / den;
                result._y = q._y / den;
        ...
        {
            if (axis.Dot() == 0.0f) return Identity;

...
namespace BrawlLib.IO
{
    public abstract class FileMap : IDisposable
    {
        ...
        {
            return FromTempFile(length, out var path);
        }

        public static FileMap FromTempFile(int length, out string path)
        {
            var stream = new FileStream(path = Path.GetTempFileName(), FileMode.Open, FileAccess.ReadWrite,
    ...
    public class wFileMap : FileMap
    {
        ...
        {
            var maxSize = offset + length;
            var maxHigh = (uint) (maxSize >> 32);
            var maxLow = (uint) maxSize;
    ...
    public class cFileMap : FileMap
    {
        ...
        {
            var cProtect = protect == FileMapProtect.ReadWrite
        ...
        public override void Dispose()
        {
            if (_mappedFile != null) _mappedFile.Dispose();

            if (_mappedFileAccessor != null) _mappedFileAccessor.Dispose();

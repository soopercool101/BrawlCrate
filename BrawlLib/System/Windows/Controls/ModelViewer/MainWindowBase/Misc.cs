...
namespace System.Windows.Forms
{
    public partial class ModelEditorBase : UserControl
    {
        ...
        private void RenderToGIF(List<Image> images, string path)
        {
            ...
            e.SetRepeat(0);
            e.SetQuality(10);
            using (var progress = new ProgressWindow(this, "GIF Encoder", "Encoding, please wait...", true))
            {
            ...
            if (MessageBox.Show(this,
                    ...
                    "GIF saved", MessageBoxButtons.YesNo) == DialogResult.Yes) Process.Start("explorer.exe", path);
        }

        protected void SaveBitmap(Bitmap bmp, string path, string extension)
        {
            if (string.IsNullOrEmpty(path)) path = Application.StartupPath + "\\ScreenCaptures";
        ...
        {
            if (_targetModels == null) _targetModels = new List<IModel>();

            var modelList = ModelPanel.CollectModels(node);
            foreach (var m in modelList) AppendTarget(m);

            ModelPanel.RefreshReferences();

            if (TargetModel == null && _targetModels.Count > 0) TargetModel = _targetModels[0];
        }
        ...
        public virtual void OpenFile(string file, bool models = true, bool animations = true, bool etc = true)
        {
            ...
            try
            {
                if ((node = NodeFactory.FromFile(null, file)) != null)
                {
                    ...
                    if (models) LoadModels(node);

                    if (animations) LoadAnimations(node);
        ...
        protected void PostConstruct()
        {
            ...
            foreach (var key in _hotkeyList)
            {
                if (key._keyDown) _hotKeysDown.Add(key.KeyCode, key._function);

                if (key._keyUp) _hotKeysUp.Add(key.KeyCode, key._function);
            }
        }
        ...
        {
            if (newModel != null && !_targetModels.Contains(newModel)) _targetModels.Add(newModel);

            if (_targetModel != null) _targetModel.IsTargetModel = false;
        ...
        {
            var val = (int) PlaybackPanel.numFrameIndex.Value;
            if (val != _animFrame)
            {
                var difference = val - _animFrame;
                if (TargetAnimation != null) SetFrame(_animFrame + difference);
            }
        }
        ...
        {
            if (TargetAnimation == null || _updating) return;

            var max = (int) PlaybackPanel.numTotalFrames.Value;
            PlaybackPanel.numFrameIndex.Maximum = max;

            if (Interpolated.Contains(TargetAnimation.GetType()) && TargetAnimation.Loop) max--;

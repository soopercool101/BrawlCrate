...
namespace System.Windows.Forms
{
    public partial class ModelEditorBase : UserControl
    {
        ...
        protected virtual void modelPanel1_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            var temp = TargetModel != null && TargetModel is MDL0Node;
            if (temp)
            {
                var m = TargetModel as MDL0Node;
                m._dontUpdateMesh = false;
            }

            if (e.Button == MouseButtons.Left)
            {
                var panel = sender as ModelPanel;

                panel.CurrentViewport.AllowSelection = true;

                if (_vertexSelection.IsMoving() && VertexLoc.HasValue) VertexChange(_selectedVertices);
        ...
        private void MouseDownTargetBone(MouseEventArgs e, ModelPanel panel)
        {
            ...
            if (SelectedBone != null)
            {
                ...
                if (!_boneSelection.SnappingAny() && !_boneSelection._hiSphere)
                {
                    ...
                    if (_hiVertex == null && !Ctrl) SelectedBone = null;

                    goto GetBone;
                }

                //Bone re-targeted. Get frame values and local point aligned to snapping plane.
                if (GetTransformPoint(new Vector2(e.X, e.Y), out var point, panel.CurrentViewport, GetBoneWorldMtx(),
                    _boneSelection))
                {
                    _boneSelection.Update(ControlType, point, GetBoneInvWorldMtx() * point, SelectedBone.FrameState);
                    if (_boneSelection.IsMoving()) BoneChange(SelectedBone);
            ...
            if (SelectedBone == null && panel.RenderBones) SelectedBone = _hiBone;
        }
        ...
        public bool MouseDownTargetVertex(MouseEventArgs e, ModelPanel panel)
        {
            ...
            if (VertexLoc.HasValue)
            {
                ...
                {
                    if (NotCtrlAlt && SelectedBone == null) ClearSelectedVertices();
                ...
                if (GetTransformPoint(new Vector2(e.X, e.Y), out var point, panel.CurrentViewport, VertexLoc.Value,
                    _vertexSelection))
                {
                    panel.CurrentViewport.AllowSelection = false;
                    ...
                    if (_vertexSelection.IsMoving()) VertexChange(_selectedVertices);
                }
            }
            else
            {
            ...
            if (ok)
            {
                ...
                if (verticesChanged) OnSelectedVerticesChanged();
            }
        ...
        protected virtual void modelPanel1_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            var panel = sender as ModelPanel;
            var viewport = panel.CurrentViewport;

            if (panel._draggingViewports) return;
            ...
            out Vector3? worldPoint)
        {
            ...
            if (GetTransformPoint(new Vector2(e.X, e.Y), out var point, viewport, localTransform, selection))
            {
                Vector3 transform = new Vector3(), lPoint;

                var coord = _coordinateTypes[(int) ControlType];
                switch (ControlType)
                {
                    case TransformType.Rotation:
                        ...
                        if (selection._lastPointLocal == lPoint) return new Vector3();

                        //Get matrix with new rotation applied
                        var m = localParentTransform * Matrix.AxisAngleMatrix(selection._lastPointLocal, lPoint);
        ...
        {
            if (SelectedBone == null) return Matrix.Identity;
        ...
        protected virtual void modelPanel1_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (_boneSelection.IsMoving() && SelectedBone != null)
            {
                var transform = GetLocalTransform(e, viewport,
                    GetBoneWorldMtx(),
                    GetBoneInvWorldMtx(),
                ...
                if (Alt && !_createdNewBone)
                    if (SelectedBone is MDL0BoneNode)
                    {
                        ...
                        var b = SelectedBone as MDL0BoneNode;
                        var model = b.Model;
                        var newBone = new MDL0BoneNode();
                        var name = "NewBone";
                        if (model != null)
                        {
                            name += "0";
                            var id = 1;
                            Top:
                            foreach (var x in model._linker.BoneCache)
                                if (x.Name == name)
                                {
                                    name = "NewBone" + id++;
                                    goto Top;
                ...
                if (point != null)
                {
                    for (var i = 0; i < 3; i++) _boneTransform[(int) ControlType](i, transform[i]);
            ...
            if (_vertexSelection.IsMoving() && VertexLoc != null)
            {
                ...
                if (point != null)
                {
                    switch (ControlType)
                    {
                        ...
                        case TransformType.Translation:
                            foreach (var vertex in _selectedVertices) vertex.WeightedPosition += transform;
            ...
            var allowHighlight = !DoNotHighlightOnMouseMove;
            var draggingSelection = viewport.Selecting;
        ...
        private delegate void MouseMoveTargetType(ModelPanel panel, MouseEventArgs e, float depth,
            ...
            SelectionParams selection)
        {
            ...
            if (ControlType == TransformType.Rotation)
            {
                ...
                if (Math.Abs(distance - radius) < radius * _selectOrbScale) //Point lies within orb radius
                {
                    ...
                    var angles = (invTransformMatrix * point).GetAngles() * Maths._rad2degf;
                    ...
                    if (Math.Abs(angles._y - 90.0f) <= _axisSnapRange)
                        ...
                        selection._hiY = true;
                    else if (angles._y >= 180.0f - _axisSnapRange || angles._y <= _axisSnapRange) selection._hiZ = true;
                }
                ...
                if (selection._hiX || selection._hiY || selection._hiZ || selection._hiCirc) snapFound = true;
            }
            else if (ControlType == TransformType.Translation || ControlType == TransformType.Scale)
            {
                ...
                foreach (var diff in testDiffs)
                {
                    var errorRange = _axisSelectRange;
                    ...
                    if (snapFound = selection._hiX || selection._hiY || selection._hiZ) break;
                }
                ...
                if (!snapFound)
                    foreach (var diff in testDiffs)
                    {
                        ...
                        else if (ControlType == TransformType.Scale)
                        {
                            ...
                            var halfDist = _scaleHalf2LDist;
                            var centerDist = _scaleHalf1LDist;
        ...
        private void GetBone(ModelPanel panel, MouseEventArgs e, float depth, ModelPanelViewport v)
        {
            if (!_boneSelection.IsMoving() && depth < 1.0f)
            {
                ...
                var point = v.UnProject(e.X, e.Y, depth);
                var doScale = v._renderAttrib._scaleBones;

                //Find orb near chosen point
                if (EditingAll)
                    foreach (var m in _targetModels)
                    foreach (var b in m.RootBones)
                        if (CompareBoneDistanceRecursive(b, point, ref o, v, doScale))
                            break;
                else if (_targetModel != null)
                    foreach (var b in _targetModel.RootBones)
                        if (CompareBoneDistanceRecursive(b, point, ref o, v, doScale))
                            break;
        ...
        private void GetVertex(ModelPanel panel, MouseEventArgs e, float depth, ModelPanelViewport v)
        {
            ...
            if (RenderVertices)
            {
                ...
                {
                    if (NotCtrlAlt) ClearSelectedVertices();
                    ...
                    if (TargetModel != null)
                    {
                        ...
                        {
                            foreach (var o in TargetModel.Objects)
                                if (o.IsRendering)
                                {
                        ...
                        {
                            var w = TargetModel.Objects[TargetModel.SelectedObjectIndex];
                            if (w.IsRendering)
                            {
                            ...
                            {
                                foreach (var h in TargetModel.Objects)
                                    if (h.IsRendering)
                                    {
                    ...
                    else if (_targetModels != null)
                    {
                        foreach (var m in _targetModels)
                            ...
                            {
                                var w = m.Objects[m.SelectedObjectIndex];
                                if (w.IsRendering)
                                {
                                    SelectVertices(w, panel);
                                    selected = true;
                    ...
                    if (selected) OnSelectedVerticesChanged();
                }
                else
                {
                    ...
                    {
                        var point = v.UnProject(e.X, e.Y, depth);
                        var vertex = CompareVertexDistance(point);
                        var update = false;
        ...
        public void HighlightStuff(MouseEventArgs e, ModelPanel panel)
        {
            ...
            var depth = panel.GetDepth(e.X, e.Y);
            var v = panel.HighlightedViewport;

            foreach (var targetFunc in _mouseMoveTargetType) targetFunc(panel, e, depth, v);
        ...
        private void SelectVertices(IObject o, ModelPanel panel)
        {
            foreach (var v in o.Vertices)
            {
                ...
                var screenTemp = panel.CurrentViewport.Camera.Project(v.WeightedPosition);
                var screenPos = (Vector2) screenTemp;

                //This is the absolute depth value, regardless of obstructions
                var vertexDepth = screenTemp._z;
                    ...
                    //if (Math.Abs(panel.GetDepth(screenPos) - vertexDepth) < 0.001f)
                {
                    ...
                    else if (!v._selected) SelectVertex(v, true);
                }
            }
        }
        ...
        public void SelectVertex(Vertex3 v, bool selected)
        {
            ...
            {
                if (!_selectedVertices.Contains(v)) _selectedVertices.Add(v);
            }
            else
            {
                if (_selectedVertices.Contains(v)) _selectedVertices.Remove(v);
            }
        }
        ...
        public void SelectAllVertices(IModel mdl)
        {
            ...
            {
                var o = mdl.Objects[mdl.SelectedObjectIndex];
                if (o.IsRendering)
                    foreach (var v in o.Vertices)
                        SelectVertex(v, true);
            }
        ...
        public void SetSelectedVertices(List<Vertex3> list)
        {
            ...
            foreach (var v in _selectedVertices) v.Selected = true;

    ...
    public class BonesPanel : UserControl
    {
        ...
        public void Reset()
        {
            ...
            //else
            {
                ...
                if (TargetModel != null)
                    foreach (var bone in TargetModel.BoneCache)
        ...
        {
            var text = txtSearchBone.Text;
            var addAll = string.IsNullOrEmpty(text) || txtSearchBone.ForeColor == Color.Gray;
        ...
        private void PopulateBoneTree()
        {
            if (TargetModel != null)
            {
                ...
                foreach (var bone in TargetModel.RootBones) RecursivePopulate(bone, boneTree.Nodes);
            }
        ...
        {
            var node = new TreeNode {Tag = bone, Text = bone.Name, Checked = bone.IsRendering};

            _treeNodes[bone.BoneIndex] = node;
            nodes.Add(node);

            foreach (IBoneNode b in ((ResourceNode) bone).Children) RecursivePopulate(b, node.Nodes);
        }
        ...
        {
            if (SelectedBone != null) SelectedBone.BoneColor = SelectedBone.NodeColor = Color.Transparent;
        ...
        {
            if (SelectedBone != null) new RenameDialog().ShowDialog(this, (ResourceNode) SelectedBone);
        }
        ...
        {
            if (!chkFlat.Checked && boneTree.SelectedNode != null) SetBone(boneTree.SelectedNode.Tag as IBoneNode);
        }
        ...
        {
            var node = SelectedBone as ResourceNode;
        ...
        private void lstBones_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            ...
            var i = e.Index;
            if (i < 0 || i >= TargetModel.BoneCache.Length) return;

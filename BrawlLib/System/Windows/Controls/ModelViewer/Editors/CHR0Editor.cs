    ...
    public class CHR0Editor : UserControl
    {
        ...
        public unsafe void ResetBox(int index)
        {
            ...
            if (TargetBone != null)
            {
                ...
                {
                    var e = entry.Keyframes.GetKeyframe(index, CurrentFrame - 1);
                    if (e == null)
                    {
        ...
        public unsafe void BoxChanged(object sender, EventArgs e)
        {
            ...
            if (SelectedAnimation != null && CurrentFrame >= 1)
            {
                var entry = SelectedAnimation.FindChild(bone.Name, false) as CHR0EntryNode;
                if (entry == null)
                {
        ...
        private void btnCopyAll_Click(object sender, EventArgs e)
        {
            ...
            if (CurrentFrame < 1)
                ...
                {
                    var frame = (CHRAnimationFrame) bone._bindState;
                    if (!AllTrans.Checked) frame.Translation = new Vector3();

                    if (!AllRot.Checked) frame.Rotation = new Vector3();

                    if (!AllScale.Checked) frame.Scale = new Vector3(1);

                    for (var i = 0; i < 9; i++) frame.SetBool(i, true);
                ...
                {
                    var frame = entry.GetAnimFrame(CurrentFrame - 1);
                    if (!AllTrans.Checked) frame.Translation = new Vector3();
        ...
        private unsafe void btnPasteAll_Click(object sender, EventArgs e)
        {
            ...
            if (CurrentFrame < 1)
                ...
                {
                    var f = _copyAllState[bone._name];
                    if (!_onlyKeys)
                    {
                        if (AllTrans.Checked) bone._bindState._translate = f.Translation;

                        if (AllRot.Checked) bone._bindState._rotate = f.Rotation;

                        if (AllScale.Checked) bone._bindState._scale = f.Scale;
                    }
                    else
                    {
                        var s = bone._bindState;
                        var sPtr = (float*) &f;
                        var dPtr = (float*) &s;
                        for (var x = 0; x < 9; x++)
                            if (f.GetBool(x))
                    ...
                    if (chkUpdateBindPose.Checked) bone.RecalcBindState(true, !chkMoveBoneOnly.Checked);

                    bone.SignalPropertyChange();
                }
            else
                foreach (var name in _copyAllState.Keys)
                {
                    ...
                    var f = _copyAllState[entry._name];
                    var i = CurrentFrame - 1;
                    var ptr = (float*) &f;
                    if (!_onlyKeys)
                    {
                        if (AllTrans.Checked) entry.SetKeyframeOnlyTrans(i, f);

                        if (AllRot.Checked) entry.SetKeyframeOnlyRot(i, f);

                        if (AllScale.Checked) entry.SetKeyframeOnlyScale(i, f);
                    }
                    else
                    {
                        for (var x = 0; x < 9; x++)
        ...
        private void btnClear_Click(object sender, EventArgs e)
        {
            ...
            foreach (CHR0EntryNode entry in SelectedAnimation.Children)
            {
                if (AllTrans.Checked) entry.RemoveKeyframeOnlyTrans(CurrentFrame - 1);

                if (AllRot.Checked) entry.RemoveKeyframeOnlyRot(CurrentFrame - 1);

                if (AllScale.Checked) entry.RemoveKeyframeOnlyScale(CurrentFrame - 1);
            }
        ...
        private void ctxBox_Opening(object sender, CancelEventArgs e)
        {
            if (SelectedAnimation == null || numRotX.Enabled == false || numRotY.Enabled == false ||
                numRotZ.Enabled == false) e.Cancel = true;
        }
        ...
        public void UpdateInterpolationEditor(NumericInputBox box)
        {
            ...
            {
                var entry = SelectedAnimation.FindChild(TargetBone.Name, false) as CHR0EntryNode;
                var kfe = entry.GetKeyframe((int) box.Tag, CurrentFrame - 1);
        ...
        private void toolStripMenuItem3_Click(object sender, EventArgs e)
        {
            ...
            KeyframeEntry kfe;
            var _target = SelectedAnimation.FindChild(TargetBone.Name, false) as CHR0EntryNode;
            if (_target != null)
            {
        ...
        {
            var frame = new CHRAnimationFrame();
            var p = (float*) &frame;
        ...
        private unsafe void btnPaste_Click(object sender, EventArgs e)
        {
            ...
            {
                var o = da.GetData("AnimationFrame");
                if (o != null && o is CHRAnimationFrame)
                {
                    var frame = (CHRAnimationFrame) o;
        ...
        private void bakeVertexPositionsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (TargetModel != null && TargetModel is MDL0Node && TargetModel.Objects != null)
            {
                ...
                {
                    for (var i = 0; i < o._manager._vertices.Count; i++)
                    {
                        var vec = o._manager._vertices[i];
                    ...
                    if (o._vertexNode.Format == WiiVertexComponentType.Float) o._vertexNode.ForceFloat = true;
                }
        ...
        private void InitializeComponent()
        {
            ...
            grpTransform.Size = new Drawing.Size(422, 78);
            ...
            // btnPaste
            // 
            btnPaste.Location = new Drawing.Point(318, 54);
            btnPaste.Name = "btnPaste";
            btnPaste.Size = new Drawing.Size(48, 20);
            ...
            // chkUpdateBindPose
            // 
            chkUpdateBindPose.Location = new Drawing.Point(154, -1);
            chkUpdateBindPose.Name = "chkUpdateBindPose";
            chkUpdateBindPose.Size = new Drawing.Size(146, 17);
            ...
            // btnCopy
            // 
            btnCopy.Location = new Drawing.Point(318, 35);
            btnCopy.Name = "btnCopy";
            btnCopy.Size = new Drawing.Size(48, 20);
            btnCopy.TabIndex = 22;
            btnCopy.Text = "Copy";
            ...
            chkMoveBoneOnly.Location = new Drawing.Point(302, -1);
            chkMoveBoneOnly.Name = "chkMoveBoneOnly";
            chkMoveBoneOnly.Size = new Drawing.Size(113, 17);
            ...
            grpTransAll.Size = new Drawing.Size(160, 78);
            ...
            // btnInsert
            // 
            btnInsert.Location = new Drawing.Point(106, 54);
            ...
            // btnClean
            // 
            btnClean.Location = new Drawing.Point(106, 35);
            ...
            // btnCopyAll
            // 
            btnCopyAll.Location = new Drawing.Point(57, 54);
            ...
            // btnClearAll
            // 
            btnClearAll.Location = new Drawing.Point(106, 16);
            btnClearAll.Name = "btnClearAll";
            btnClearAll.Size = new Drawing.Size(50, 20);
            ...
            ctxTools.Size = new Drawing.Size(186, 26);
            // 
            // bakeVertexPositionsToolStripMenuItem
            // 
            bakeVertexPositionsToolStripMenuItem.Name = "bakeVertexPositionsToolStripMenuItem";
            bakeVertexPositionsToolStripMenuItem.Size = new Drawing.Size(185, 22);
            ...
            numScaleZ.Size = new Drawing.Size(82, 20);
            ...
            numRotX.Size = new Drawing.Size(82, 20);
            numRotX.TabIndex = 15;
            numRotX.Text = "0";
            ...
            numRotY.Size = new Drawing.Size(82, 20);
            numRotY.TabIndex = 16;
            numRotY.Text = "0";
            ...
            numRotZ.Size = new Drawing.Size(82, 20);
            numRotZ.TabIndex = 17;
            numRotZ.Text = "0";
            ...
            numTransZ.Size = new Drawing.Size(82, 20);
            ...
            MinimumSize = new Drawing.Size(582, 78);
            Name = "CHR0Editor";
            Size = new Drawing.Size(582, 78);

...
namespace Be.Windows.Forms
{
    ...
    public class HexBox : Control
    {
        ...
        public SectionEditor SectionEditor { get; set; }
        ...
        public class KeyInterpreter : IKeyInterpreter
        {
            ...
            protected virtual bool PreProcessWmKeyDown_Down(ref Message m)
            {
                ...
                if (pos == _hexBox._byteProvider.Length && cp == 0) return true;
            ...
            protected virtual bool PreProcessWmKeyDown_ShiftUp(ref Message m)
            {
                ...
                if (pos - _hexBox._iHexMaxHBytes < 0 && pos <= _bpiStart.Index) return true;
            ...
            protected virtual bool PreProcessWmKeyDown_Back(ref Message m)
            {
                ...
                if (startDelete < 0 && sel < 1) return true;
                ...
                if (sel == 0) PerformPosMoveLeftByte();
            ...
            {
                if (ModifierKeys == Keys.Control) return _hexBox.BasePreProcessMessage(ref m);
                ...
                if (Uri.IsHexDigit(c))
                {
                    ...
                    if (!isInsertMode && si && _hexBox.InsertActive && cp == 0) isInsertMode = true;
        ...
        private class StringKeyInterpreter : KeyInterpreter
        {
            ...
            public override bool PreProcessWmChar(ref Message m)
            {
                ...
                // do insert when insertActive = true
                if (!isInsertMode && si && _hexBox.InsertActive) isInsertMode = true;
        ...
        private void UpdateScrollSize()
        {
            ...
            if (VScrollBarVisible && _byteProvider != null && _byteProvider.Length > 0 && _iHexMaxHBytes != 0)
            {
                ...
                if (scrollmax == _scrollVmax && scrollpos == _scrollVpos) return;
        ...
        {
            var max = 65535;
            if (_scrollVmax < max) return value;
        ...
        {
            if (pos < _scrollVmin || pos > _scrollVmax || pos == _scrollVpos) return;
        ...
        private void PerformScrollThumpPosition(long pos)
        {
            ...
            if (ToScrollPos(pos) == ToScrollMax(_scrollVmax) - difference) pos = _scrollVmax;
            // End Bug fix
        ...
        private BytePositionInfo GetHexBytePositionInfo(Point p)
        {
            ...
            if (byteCharaterPos > 1) byteCharaterPos = 1;

            if (bytePos == _byteProvider.Length) byteCharaterPos = 0;
        ...
        public long FindNext(FindOptions options)
        {
            ...
            for (var pos = startIndex; pos < _byteProvider.Length; pos++)
            {
                ...
                var compareByte = _byteProvider.ReadByte(pos);
                var buffer1Match = compareByte == buffer1[match];
                var hasBuffer2 = buffer2 != null;
                var buffer2Match = hasBuffer2 ? compareByte == buffer2[match] : false;
                var isMatch = buffer1Match || buffer2Match;
                if (!isMatch)
                {
                    pos -= match;
                    match = 0;
                ...
                {
                    var bytePos = pos - buffer1Length + 1;
        ...
        public long FindPrev(FindOptions options)
        {
            ...
            for (var pos = startIndex; pos >= 0; pos--)
            {
                if (_abortFind) return -2;
                ...
                var compareByte = _byteProvider.ReadByte(pos);
                var buffer1Match = compareByte == buffer1[buffer1.Length - 1 - match];
                var hasBuffer2 = buffer2 != null;
                var buffer2Match = hasBuffer2 ? compareByte == buffer2[buffer2.Length - 1 - match] : false;
                var isMatch = buffer1Match || buffer2Match;
                if (!isMatch)
                {
                    pos += match;
        ...
        public void Paste()
        {
            ...
            else
            {
                ...
                foreach (var c in s)
                {
                    if (Array.IndexOf(hexChars, c) != -1) byteString += c;
        ...
        public void PasteHex(bool overwrite)
        {
            ...
            if (overwrite)
            {
                var length = Math.Max(_selectionLength, buffer.Length);

                if (length > 0) _byteProvider.DeleteBytes(_bytePos, length);
            }
        ...
        private void PaintHeaderRow(Graphics g)
        {
            ...
            var headerPointF = new PointF(_recLineInfo.X, _recColumnInfo.Y);

            g.DrawString(string.Format("Offset({0})", _offsetDisplay == OffsetDisplay.Hex ? "h" : "d"), Font, brush,
                headerPointF, _stringFormat);

            for (var col = 0; col < _iHexMaxHBytes; col++) PaintColumnInfo(g, (byte) col, brush, col);
        }

        private void PaintColumnSeparator(Graphics g)
        {
            for (var col = GroupSize; col < _iHexMaxHBytes; col += GroupSize)
            {
                var pen = new Pen(new SolidBrush(ColumnDividerColor), 1);
                var headerPointF = GetColumnInfoPointF(col);
        ...
        private RelCommand GetBrushes(int index, ref Brush foreColor, ref Brush backColor)
        {
            ...
            var command = s._manager.GetCommand(index);
            var cmd = command != null && !command.IsHalf;

            backColor =
                SectionEditor.SelectedRelocationIndex == index ? SelectedBrush :
                cmd ? CommandBrush :
                null;

            if (backColor == null && s.HasCode)
            {
                var code = s._manager.GetCode(index);
                var returnBranch = code is PPCblr;
                var branch = code is PPCBranch && !returnBranch;
                var linkedBranch =
                    SectionEditor.TargetBranchOffsetRelocation != null &&
                    SectionEditor.TargetBranchOffsetRelocation._index == index;
            ...
            if (backColor == null) backColor = SectionEditor._manager.GetColor(index);
        ...
        private void PaintHexStringSelected(Graphics g, byte b, Brush brush, Brush brushBack, Point gridPoint)
        {
            ...
            var isFirstLineChar = gridPoint.X == 0;
            var bcWidth = isLastLineChar ? _charSize.Width * 2.3f : _charSize.Width * 3;
        ...
        private void PaintHexAndStringView(Graphics g, long startByte, long endByte)
        {
            ...
            if (SectionEditor != null)
            {
                ...
                for (var x = startByte; x <= intern_endByte; x += 4, index++)
                {
                    var word =
                    ...
                    RelCommand cmd = null;
                    if (SectionEditor != null && (cmd = GetBrushes(index, ref foreBrush, ref backBrush)) != null)
                        word = cmd.Apply(word, 0);

                    var half = cmd != null && cmd.IsHalf;
                    for (var u = 0; u < 4; u++, offset++, counter++)
                    {
                        var gridPoint = GetGridBytePoint(counter);
                        var byteStringPointF = GetByteStringPointF(gridPoint);

                        if (half && u > 1) backBrush = CommandBrush;

                        var b = _byteProvider.ReadByte(x + u);
                        if (cmd != null && SectionEditor.displayInitialized.Checked)
                            b = (byte) ((word >> ((3 - u) * 8)) & 0xFF);

                        var isSelectedByte = offset >= _bytePos && offset <= _bytePos + _selectionLength - 1 &&
                                             _selectionLength != 0;
            ...
            {
                for (var x = startByte; x <= intern_endByte; x++, counter++)
                {
                    var gridPoint = GetGridBytePoint(counter);
                    var byteStringPointF = GetByteStringPointF(gridPoint);

                    var b = _byteProvider.ReadByte(x);
                    var isSelectedByte = x >= _bytePos && x <= _bytePos + _selectionLength - 1 && _selectionLength != 0;
            ...
            if (_shadowSelectionVisible) PaintCurrentBytesSign(g);
        }
        ...
        {
            var gp = GetGridBytePoint(col);
            var x = 3 * _charSize.Width * gp.X + _recColumnInfo.X;
        ...
        /// <summary>
        ///     Gets or sets the number of bytes in a group. Used to show the group separator line (if GroupSeparatorVisible is
        ///     true)
        /// </summary>
        /// <remarks>
        ...
        public IByteProvider ByteProvider
        {
            ...
            set
            {
                ...
                if (_byteProvider != null) _byteProvider.LengthChanged -= byteProvider_LengthChanged;
        ...
        private byte[] ConvertHexToBytes(string hex)
        {
            ...
            for (var i = 0; i < hexArray.Length; i++)
            {
                ...
                var isByte = ConvertHexToByte(hexValue, out var b);
        ...
        private void CheckCurrentPositionInLineChanged()
        {
            ...
            var currentPositionInLine = gb.X + _byteCharacterPos;

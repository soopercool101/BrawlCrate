...
namespace Be.Windows.Forms
{
    ...
    public sealed class DynamicFileByteProvider : IByteProvider, IDisposable
    {
        ...
        public DynamicFileByteProvider(Stream stream)
        {
            ...
            if (!stream.CanSeek) throw new ArgumentException("stream must supported seek operations(CanSeek)");
        ...
        private byte ReadByteFromFile(long fileOffset)
        {
            ...
            if (_stream.Position != fileOffset && fileOffset >= 0) _stream.Position = fileOffset;
        ...
        private void MoveFileBlock(FileDataBlock fileBlock, long dataOffset)
        {
            ...
            var nextFileBlock = GetNextFileDataBlock(fileBlock, dataOffset, out var nextDataOffset);
        ...
        {
            var block = GetDataBlock(index, out var blockOffset);
            var fileBlock = block as FileDataBlock;
            if (fileBlock != null) return ReadByteFromFile(fileBlock.FileOffset + index - blockOffset);
            ...
            try
            {
                ...
                if (blockOffset == index && block.PreviousBlock != null)
                {
                    ...
                    if (previousMemoryBlock != null)
                    {
                        ...
                        if (fileBlock.Length == 0) _dataMap.Remove(fileBlock);
                        return;
                    }
                }
                ...
                if (index > blockOffset) prefixBlock = new FileDataBlock(fileBlock.FileOffset, index - blockOffset);
                ...
                if (prefixBlock != null) _dataMap.AddBefore(block, prefixBlock);

                if (suffixBlock != null) _dataMap.AddAfter(block, suffixBlock);
            }
            finally
            {
            ...
            try
            {
                ...
                while (bytesToDelete > 0 && block != null)
                {
                    ...
                    if (block.Length == 0)
                    {
                        ...
                        if (_dataMap.FirstBlock == null) _dataMap.AddFirst(new MemoryDataBlock(new byte[0]));
                    }
        ...
        public bool HasChanges()
        {
            ...
            if (Length != _stream.Length) return true;

            long offset = 0;
            for (var block = _dataMap.FirstBlock; block != null; block = block.NextBlock)
            {
                var fileBlock = block as FileDataBlock;
                if (fileBlock == null) return true;

                if (fileBlock.FileOffset != offset) return true;
        ...
        {
            if (ReadOnly) throw new OperationCanceledException("File is in read-only mode");
            ...
            if (Length > _stream.Length) _stream.SetLength(Length);
            ...
            {
                var fileBlock = block as FileDataBlock;
                if (fileBlock != null && fileBlock.FileOffset != dataOffset) MoveFileBlock(fileBlock, dataOffset);

...
namespace System.Windows.Forms
{
    public class SoundPackControl : UserControl
    {
        ...
        private void Update(ListView list)
        {
            ...
            list.Items.Clear();
            if (_targetNode != null)
                foreach (var file in _targetNode.Files)
        ...
        private void mnuPath_Click(object sender, EventArgs e)
        {
            ...
            {
                var ext = _selectedItem._node as RSARExtFileNode;
        ...
        private void lstSets_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (audioPlaybackPanel1.TargetSource != null) audioPlaybackPanel1.TargetSource = null;

            if (_grid != null && _selectedItem != null) _grid.SelectedObject = _selectedItem._node;
        }
        ...
        private void mnuExport_Click(object sender, EventArgs e)
        {
            if (_selectedItem.SubItems[1].Text != "External")
                using (var dlg = new SaveFileDialog())
                {
                    ...
                    if (dlg.ShowDialog(this) == DialogResult.OK) _selectedItem._node.Export(dlg.FileName);
                }
        }
        ...
        private void lstSets_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape)
            {
                ...
                if (_grid != null) _grid.SelectedObject = _targetNode;
            }
        }
        ...
        private void mnuReplace_Click(object sender, EventArgs e)
        {
            using (var dlg = new OpenFileDialog())
            {
                ...
                if (dlg.ShowDialog() == DialogResult.OK) _selectedItem._node.Replace(dlg.FileName);
            }
        }
        ...
        {
            using (var ofd = new OpenFileDialog {Filter = SupportedFilesHandler.GetCompleteFilter("wav")})
            {
                if (ofd.ShowDialog() == DialogResult.OK)
                {
                    var r = new RSTMNode {_fileIndex = _targetNode.Files.Count};
        ...
        private void makeAllExternalToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                if (i._node is RSARExtFileNode) continue;

                var v = i as ListViewItem;
                var type = v.SubItems[1].Text;
                var dir = "\\" + type + "\\";
                var fileName = i._node.Name.Replace('/', '_').Replace('<', '(').Replace('>', ')') + ".b" +
                               type.ToLower();

                var newPath = _targetNode._origPath.Substring(0, _targetNode._origPath.LastIndexOf('\\')) + dir;
                if (!Directory.Exists(newPath)) Directory.CreateDirectory(newPath);

                i._node.Export(newPath + fileName);

                var groups = i._node.GroupRefNodes;

                //Contains lists of all instances of this file in each group
                var refs = new List<int>[groups.Length];
                ...
                for (var n = 0; n < groups.Length; ++n)
                {
                    refs[n] = new List<int>();
                    var g = groups[n];
                    for (var m = 0; m < g._files.Count; ++m)
                ...
                {
                    var rbnk = i._node as RBNKNode;
                    rbnkRefs = rbnk._rsarBankEntries;
                    for (var j = 0; j < rbnk._rsarBankEntries.Count; ++j)
                    {
                        var b = rbnk._rsarBankEntries[j];
                        b.BankNode = null;
                    }
                    //if (i._node.Children.Count > 0)
                ...
                {
                    var rbnk = i._node as RBNKNode;
                    foreach (var b in rbnkRefs) b.BankNode = ext;
                }

                //Remake references
                for (var groupID = 0; groupID < refs.Length; ++groupID)
                {
                    var r = 0;
                    var group = groups[groupID];
                    foreach (var occurrence in refs[groupID]) group._files.Insert(occurrence + r++, ext);
        ...
        private void makeAllInternalToolStripMenuItem_Click(object sender, EventArgs e)
        {
            foreach (SoundPackItem i in lstSets.Items)
            {
                ...
                var w = i._node as RSARExtFileNode;
                var path = w.FullExtPath;
                if (!File.Exists(path)) continue;

                var ext = NodeFactory.FromFile(null, path) as RSARFileNode;
                if (ext == null) continue;

                var v = i as ListViewItem;
                var type = v.SubItems[1].ToString();
    ...
    public class SoundPackItem : ListViewItem
    {
        ...
        public SoundPackItem(RSARFileNode file)
        {
            ...
            var s = file.ResourceFileType.ToString();
            if (file is RSARExtFileNode) s = "External";

            SubItems.Add(s);
            var i = Helpers.FindFirst(file.Name, 0, ']');
            ...
            if (ColumnToSort == 0)
                ...
                compareResult =
                    ObjectCompare.Compare(
                        int.Parse(listviewX.SubItems[ColumnToSort].Text.Substring(2), NumberStyles.HexNumber),
                        int.Parse(listviewY.SubItems[ColumnToSort].Text.Substring(2), NumberStyles.HexNumber));
            else

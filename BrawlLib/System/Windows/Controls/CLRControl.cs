    ...
    public class CLRControl : UserControl
    {
        ...
        private IList<int> SelectedIndices
        {
            get
            {
                ...
                for (var i = 0; i < array.Length; i++) array[i] = indices[i];
                return array;
            }
        }
        ...
        private void SourceChanged()
        {
            ...
            {
                var count = _colorSource.ColorCount(_colorId);
                for (var i = 0; i < count; i++) lstColors.Items.Add(_colorSource.GetColor(i, _colorId));
        ...
        {
            if (e.KeyCode == Keys.Enter) lstColors_DoubleClick(sender, e);
        }
        ...
        private void lstColors_DoubleClick(object sender, EventArgs e)
        {
            ...
            if (_colorSource == null || indices.Count <= 0) return;

            var count = indices.Count;
            if (count == 1)
            {
                tempIndex = indices[0];
                var prev = (ARGBPixel) lstColors.Items[tempIndex];
            ...
            else
            {
                ...
                if (_dlgGradient.ShowDialog(this) == DialogResult.OK)
                {
                    ...
                    var start = (ARGBPixel) _dlgGradient.StartColor;
                    var end = (ARGBPixel) _dlgGradient.EndColor;
                    var stepA = (end.A - start.A) / (float) count;
                    var stepR = (end.R - start.R) / (float) count;
                    var stepG = (end.G - start.G) / (float) count;
                    var stepB = (end.B - start.B) / (float) count;
        ...
        private void lstColors_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            if (index >= 0)
            {
                var p = (ARGBPixel) lstColors.Items[index];
                var boxWidth = 40;
                var textWidth = 275;
                ...
                var n = Math.Floor(Math.Log10(_colorSource.ColorCount(_colorId)) + 1);
        ...
        private void copyToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var s = "";
                foreach (ARGBPixel w in lstColors.SelectedItems) s += w.ToRGBAColorCode() + Environment.NewLine;
        ...
        private void lstColors_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                var index = lstColors.IndexFromPoint(e.Location);
        ...
        private void CLRControl_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            test = node =>
            {
                if (node.ShortcutKeys == e.KeyData) node.PerformClick();

                foreach (ToolStripMenuItem child in node.DropDownItems) test(child);
            };
            foreach (ToolStripMenuItem item in ctxMenu.Items) test(item);
        }

        private void selectAllToolStripMenuItem_Click(object sender, EventArgs e)
        {
            lstColors.BeginUpdate();
            for (var i = 0; i < lstColors.Items.Count; i++) lstColors.SetSelected(i, true);
        ...
        private void allToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            {
                var v = 0;
                var pixels = GetCopiedPixels();
                foreach (var r in indices)
                {
                    if (v >= pixels.Count) break;
        ...
        private void colorToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (indices.Count >= 0)
            {
                ...
                foreach (var r in indices)
                {
                    ...
                    var copied = pixels[v++];
                    var temp = (ARGBPixel) lstColors.Items[r];
                    temp.R = copied.R;
                    temp.G = copied.G;
                    temp.B = copied.B;

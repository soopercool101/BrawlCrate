...
namespace System.Windows.Forms
{
    public unsafe partial class TexCoordRenderer : GLPanel
    {
        ...
        public BindingList<string> UVSetNames { get; } = new BindingList<string>();

        public BindingList<string> ObjectNames { get; } = new BindingList<string>();
        ...
        {
            if (_targetMatRef != texture && texture != null) CurrentViewport.Camera.Reset();

            if (_attached == null) _attached = new List<ResourceNode>();

            if (Tex0 != null) Tex0.Unbind();

            _attached.Clear();

            _targetMatRef = texture;
            if (_targetMatRef == null) return;

            _attached.Add(_targetMatRef);

            if (Tex0 != null) Tex0.Prepare(_targetMatRef, -1);

            //Dispose of all old UV buffers
            if (_renderInfo != null)
                foreach (var info in _renderInfo)
            ...
            var coordID = _targetMatRef.TextureCoordId;
            if (coordID < 0)
            {
            ...
            foreach (var obj in _targetMatRef.Material.Objects)
            {
                ObjectNames.Add(obj.Name);
                var info = new RenderInfo(obj._manager);
        ...
        {
            _uvIndex = UVSetNames.Count == 1 ? 0 : index >= 0 ? index.Clamp(0, UVSetNames.Count - 2) : -1;
        ...
        {
            _uvSetIndices.Clear();
            if ((_objIndex = ObjectNames.Count == 1 ? 0 : index >= 0 ? index.Clamp(0, ObjectNames.Count - 2) : -1) >= 0)
            {
                if (_targetMatRef.Material.Objects.Length != 0)
                    foreach (var uv in _targetMatRef.Material.Objects[_objIndex]._uvSet)
                        if (uv != null)
            ...
            {
                var model = _targetMatRef.Model;
                string name = null;
                if (UVSetNames.Count > 0 && _uvIndex >= 0 && _uvIndex < UVSetNames.Count) name = UVSetNames[_uvIndex];

                UVSetNames.Clear();
                UVSetNames.Add(_uvSetIndices.Count == 1 ? model._uvList[_uvSetIndices[0]].Name : "All");
                if (model != null && model._uvList != null && _uvSetIndices.Count != 1)
                    foreach (var i in _uvSetIndices)
                        UVSetNames.Add(model._uvList[i].Name);

                SetUVIndex(string.IsNullOrEmpty(name) ? UVSetNames.IndexOf(name) : -1);
            }
        ...
        {
            var singleObj = _objIndex >= 0;
            var singleUV = _uvIndex >= 0;
            var r = 0;
            for (var i = 0; i < _renderInfo.Count; i++)
            {
        ...
        public void Render(GLCamera cam, bool renderBG)
        {
            ...
            var texture = GLTex;
            if (texture == null || texture._texId <= 0) return;
            ...
            var tAspect = texWidth / texHeight;
            var wAspect = (float) Width / Height;

            var texCoord = new float[8];
            ...
            var state = _targetMatRef._bindState;
            ...
            var trans = new OpenTK.Vector3(-topLeft._x / Width * xCorrect, topLeft._y / Height * yCorrect, 0.0f);
            GL.Translate(trans);
            ...
            var mdlScale = new Vector2(bottomRight._x - topLeft._x, bottomRight._y - topLeft._y);
            ...
            foreach (var info in _renderInfo) info.PrepareStream();
        }

        protected override void OnRender(PaintEventArgs e)
        {
            var r = CurrentViewport.Region;
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            if (_ctx != null && _grabbing)
                lock (_ctx)
                {
                    var xDiff = e.X - _lastX;
                    var yDiff = e.Y - _lastY;
        ...
        {
            if (e.Button == MouseButtons.Right) _grabbing = true;
        ...
        {
            var scale = amt >= 0 ? amt / 2.0f : 2.0f / -amt;
        ...
        protected override bool ProcessKeyMessage(ref Message m)
        {
            ...
            if (m.Msg == 0x100)
            {
                ...
                var ctrl = (mod & Keys.Control) != 0;
                var shift = (mod & Keys.Shift) != 0;
                var alt = (mod & Keys.Alt) != 0;
        ...
        private class RenderInfo
        {
            ...
            public readonly PrimitiveManager _manager;
            ...
            {
                _manager = manager;
                for (var i = 0; i < 8; i++) _enabled[i] = true;
            ...
            public void CalcMinMax()
            {
                ...
                if (_manager != null && _manager._faceData != null)
                    for (var i = 4; i < _manager._faceData.Length; i++)
                        if (_manager._faceData[i] != null && _enabled[i - 4])
                        {
                            var pSrc = (Vector2*) _manager._faceData[i].Address;
                            for (var x = 0; x < _manager._pointCount; x++, pSrc++)
                            {
                                _min.Min(*pSrc);
                                _max.Max(*pSrc);
                            }
            ...
            public void PrepareStream()
            {
                ...
                CalcStride();
                var bufferSize = _manager._pointCount * _stride;
                ...
                {
                    _dirty = false;
                    for (var i = 0; i < 8; i++) UpdateStream(i);
                }

                GL.EnableClientState(ArrayCap.VertexArray);

                var pData = (Vector2*) _renderBuffer.Address;
                for (var i = 4; i < _manager._faceData.Length; i++)
                    if (_manager._faceData[i] != null && _enabled[i - 4])
                        GL.VertexPointer(2, VertexPointerType.Float, _stride, (IntPtr) pData++);

                if (_manager._triangles != null) _manager._triangles.Render();

                if (_manager._lines != null) _manager._lines.Render();

                if (_manager._points != null) _manager._points.Render();
            ...
            {
                index += 4;
                if (_manager._faceData[index] == null || !_enabled[index - 4]) return;

                //Set starting address
                var pDst = (byte*) _renderBuffer.Address;
                for (var i = 4; i < index; i++)
                    if (_manager._faceData[i] != null && _enabled[i - 4])
                        pDst += 8;

                var pSrc = (Vector2*) _manager._faceData[index].Address;
                for (var i = 0; i < _manager._pointCount; i++, pDst += _stride) *(Vector2*) pDst = *pSrc++;
            }
        }
    }
}
...
namespace System.Audio
{
    ...
    internal unsafe struct RIFFHeader
    {
        ...
        public smplLoop[] _smplLoops
        {
            get
            {
                ...
                var ptr = Address + 12;
                var end = Address + _length;
                while (ptr < end)
                {
                    // There is more data in the file
                    var chunk = (smplChunk*) ptr;
        ...
        public RIFFHeader(int format, int channels, int bitsPerSample, int sampleRate, int numSamples)
        {
            ...
            var dataLen = (uint) (numSamples * _fmtChunk._blockAlign);
            _length = 0;
    ...
    internal unsafe struct smplChunk
    {
        ...
        public smplLoop[] _smplLoops
        {
            get
            {
                var arr = new smplLoop[_cSampleLoops];
                for (var i = 0; i < arr.Length; i++)
                {
                    var thisaddr = Address;
                    var cc = (smplChunk*) Address;
                    var thataddr = Address + 0x2C + i * 0xC;
    ...
    public static unsafe class WAV
    {
        ...
        {
            var sampleCount = Math.Min(maxSampleCount, source.Samples - samplePosition);

            //Estimate size
            var outLen = 44 + sampleCount * source.Channels * 2;
            if (appendSmplChunk && source.IsLooping) outLen += 68;

            //Create byte array
            var wavData = new byte[outLen];
            fixed (byte* address = wavData)
            {
                var riff = (RIFFHeader*) address;
                *riff = new RIFFHeader(1, source.Channels, 16, source.Frequency, sampleCount);
                ...
                {
                    riff->_length += 68;
                    var smpl = (smplChunk*) (address + outLen - 68);
                    *smpl = new smplChunk
                    {
                    ...
                    var loop = (smplLoop*) (address + outLen - 24);
                    *loop = new smplLoop
                    {
                        _dwIdentifier = 0,
                        _dwType = 0,

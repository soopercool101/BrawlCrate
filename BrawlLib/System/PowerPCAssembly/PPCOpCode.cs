...
namespace System.PowerPcAssembly
{
    public abstract partial class PPCOpCode : ICloneable
    {
        ...
        {
            if (_names.Count > 0) return _names[0];
        ...
        {
            var formatted = _operands.Select(x => x.GetFormatted()).ToArray();
    ...
    public class PPCBranch : PPCOpCode
    {
        ...
        public string Offset
        {
            ...
            set
            {
                ...
                if (int.TryParse(s, NumberStyles.HexNumber, null, out var offset)) DataOffset = offset * (neg ? -1 : 1);
            }
        }
    ...
    public class PPCBc : PPCBranch
    {
        ...
        public override string GetName()
        {
            if (IgnoreCr && IgnoreCtr) return _names[0];

            var name = "";
            if (!IgnoreCr && IgnoreCtr)
                name = _names[3 + CompareType * 2 + (CrState ? 1 : 0)];
            else if (IgnoreCr && !IgnoreCtr) name = _names[1 + (CtrState ? 1 : 0)];

            if (!IgnoreCr && !IgnoreCtr) name = _names[1 + (CtrState ? 1 : 0)] + (CrState ? "t" : "f");
    ...
    public class PPCOr : PPCOpCode
    {
        ...
        public override string GetName()
        {
            if (_operands[1].Value == _operands[2].Value) return _names[1];
    ...
    public class PPCOri : PPCOpCode
    {
        ...
        public override string GetFormattedOperands()
        {
            if (!Exclusive && _operands[0].Value == 0 && _operands[1].Value == 0 && _operands[2].Value == 0) return "";
    ...
    public class PPCLwz : PPCOpCode
    {
        ...
        public override string GetName()
        {
            if (LoadDouble) return _names[5];

            if (LoadMultiple) return _names[4];
    ...
    public class PPCStw : PPCOpCode
    {
        ...
        public override string GetName()
        {
            if (StoreDouble) return _names[4];

            if (StoreMultiple) return _names[3];
            ...
            if (StoreWord)
                ...
                name = _names[1];
            else if (StoreHalf) name = _names[2];

...
namespace BrawlLib.SSBBTypes
{
    ...
    internal unsafe struct HEADHeader
    {
        ...
        {
            RuintList* list;
            var offset = _entries.Address;
            var dataOffset = 0x60 + channels * 8;
            ...
            *(AudioFormatInfo*) list->Get(offset, 0) =
            ...
            //Fill remaining
            var p = (int*) (offset + dataOffset);
            for (dataOffset += 8; dataOffset < size; dataOffset += 4) *p++ = 0;
        }
        ...
        public ADPCMInfo[] ChannelInfo
        {
            get
            {
                var list = Part3;
                var offset = _entries.Address;
                var count = list->_numEntries._data;
                var arr = new ADPCMInfo[count];
                for (var i = 0; i < count; i++) arr[i] = *(ADPCMInfo*) ((ruint*) list->Get(offset, i))->Offset(offset);

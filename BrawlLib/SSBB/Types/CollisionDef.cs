...
namespace BrawlLib.SSBBTypes
{
    ...
    public struct ColPlane
    {
        ...
        public ColPlane(int pInd1, int pInd2, int pLink1, int pLink2, CollisionPlaneType type,
            CollisionPlaneFlags2 flags2, CollisionPlaneFlags flags, CollisionPlaneMaterial material)
        {
            _point1 = (short) pInd1;
            _point2 = (short) pInd2;
            _link1 = (short) pLink1;
    ...
    public unsafe struct ColObject
    {
        ...
        private static void SetStr(byte* dPtr, string str)
        {
            ...
            {
                //Fill string
                var len = Math.Min(str.Length, 31);
                fixed (char* p = str)
                {
                    while (index < len) *dPtr++ = (byte) p[index++];
                }
            }

            //Fill remaining
            while (index++ < 32) *dPtr++ = 0;
        }
    }
    ...
    public enum CollisionPlaneMaterial : byte
    {
        ...
        SpikesTargetTestOnly =
            0x12, // Used for Spike Hazards in Target Test levels and collision hazard #1 for SSE stages. Crashes or has no effect on stages not using a target test module

        Hazard2SSEOnly =
            0x13, // Used for hitboxes on certain SSE levels (180002). Crashes or has no effect on versus stages.

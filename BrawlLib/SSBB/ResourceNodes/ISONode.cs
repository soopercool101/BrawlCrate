...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class ISONode : ISOEntryNode
    {
        ...
        public override bool OnInitialize()
        {
            ...
            var p = Get<ISOPartLists>(0x40000);
        ...
        public override void OnPopulate()
        {
            ...
            var total = p._channelCount + pCount;

            for (var i = 0; i < total; ++i)
            {
                var offset = i < pCount ? p.PartitionOffset + i * 8L : p.ChannelOffset + (i - pCount) * 8L;
                var e = Get<PartitionTableEntry>(offset);
        ...
        {
            var header = (ISOPartitionHeader*) source.Address;
            var GCMatch = header->_tagGC == ISOPartitionHeader.GCTag;
            var WiiMatch = header->_tagWii == ISOPartitionHeader.WiiTag;
            if ((GCMatch || WiiMatch) && (LoadedKey != null || LoadKey())) return new ISONode {_isGC = GCMatch};
    ...
    public class ISOPartitionNode : ISOEntryNode, IBufferNode
    {
        ...
        [Category("TMD")] public RSAType RSA { get; private set; }

        [Category("TMD")] public byte[] RSASig { get; private set; }
        ...
        [Category("TMD")] public byte[] TitleKey { get; private set; }
        ...
        public override bool OnInitialize()
        {
            ...
            var tmdOffset = _info._tmdOffset * OffMult;
            ...
            if (sigType != 0)
            {
                ...
                var size = 0x200 - ((int) sigType & 1) * 0x100;

                RSASig = GetBytes(tmdOffset, size, true);
                ...
                for (var i = 0; i < _tmd._numContents; ++i)
                    _tmdEntries.Add(Get<TMDEntry>(tmdOffset + i * TMDEntry.Size, true));

                var encryptedKey = GetBytes(0x1BF, 16, true);
                ...
                using (var aesAlg = new AesManaged())
                {
                    ...
                    using (var msDecrypt = new MemoryStream(encryptedKey))
                    using (var csDecrypt = new CryptoStream(msDecrypt, decryptor, CryptoStreamMode.Read))
                    {
                        csDecrypt.Read(TitleKey, 0, 16);
                    }
                }
            }
        ...
        {
            if (block == _cachedBlock) return null;

            var blockData = GetBytes(_info._dataOffset + BlockSize * block, BlockSize, true);
            var enc = blockData.SubArray(BlockHeaderSize, BlockDataSize);
            var dec = new byte[BlockDataSize];
            ...
            using (var aesAlg = new AesManaged())
            {
                ...
                var decryptor = aesAlg.CreateDecryptor(aesAlg.Key, aesAlg.IV);
                using (var msDecrypt = new MemoryStream(enc))
                using (var csDecrypt = new CryptoStream(msDecrypt, decryptor, CryptoStreamMode.Read))
                {
        ...
        {
            if (RSA == RSAType.None) return GetBytes(offset, size, true);

            var buffer = new List<byte>(size);
            var block = (uint) (offset / BlockDataSize);
            var cacheOffset = (int) (offset % BlockDataSize);
            int cacheSize;

            while (size > 0)
            {
                var cache = DecryptBlock(block);

                cacheSize = size;
                if (cacheSize + cacheOffset > BlockDataSize) cacheSize = BlockDataSize - cacheOffset;
    ...
    public abstract unsafe class ISOEntryNode : ResourceNode
    {
        ...
        {
            var buffer = new UnsafeBuffer(bytes.Length);
        ...
        {
            var s = RootNode.WorkingUncompressed.Map.BaseStream;
        ...
        public void Create(ISOEntryNode parent, long offset, int size, bool relativeOffset)
        {
            ...
            _parent = parent;
            var buffer = GetBuffer(_rootOffset, size);
            _parent = null;

...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class CollisionNode : ARCEntryNode
    {
        ...
        public override bool OnInitialize()
        {
            ...
            _objects.Clear();
            var obj = Header->Objects;
            for (int i = Header->_numObjects; i-- > 0;) _objects.Add(new CollisionObject(this, obj++));
        ...
        {
            var header = (CollisionHeader*) address;
            *header = new CollisionHeader(_pointCount, _planeCount, _objects.Count, _unk1);

            var pPoint = header->Points;
            var pPlane = header->Planes;
            var pObj = header->Objects;
            ...
            foreach (var obj in _objects)
            {
                ...
                {
                    if (plane._encodeIndex != -1) continue;
                    ...
                    if (link != null)
                        ...
                        {
                            if (p == current || p._linkLeft != link) continue; //We only want to go left-to-right!
        ...
        public void Render()
        {
            ...
            foreach (var obj in _objects) obj.Render();
        }
        ...
        {
            var header = (CollisionHeader*) source.Address;

            if (header->_numPoints < 0 || header->_numPlanes < 0 || header->_numObjects < 0 ||
                header->_unk1 < 0) return null;
        ...
        {
            foreach (var co in external._objects) _objects.Add(co);
    ...
    public unsafe class CollisionObject
    {
        ...
        public CollisionObject(CollisionNode parent, ColObject* entry)
        {
            ...
            var pPlane = &parent.Header->Planes[planeOffset];

            //Decode points
            var pPtr = &parent.Header->Points[pointOffset];
            for (var i = 0; i < pointCount; i++) new CollisionLink(this, *pPtr++);
        ...
        public MDL0BoneNode LinkedBone
        {
            ...
            set
            {
                if (_linkedBone != null)
                    ...
                    {
                        if (p.LinkLeft != null) p.LinkLeft._rawValue = _linkedBone.Matrix * p.LinkLeft._rawValue;

                        if (p.LinkRight != null) p.LinkRight._rawValue = _linkedBone.Matrix * p.LinkRight._rawValue;
                    }
                ...
                if ((_linkedBone = value) != null)
                {
                    ...
                    foreach (var p in _planes)
                    {
                        if (p.LinkLeft != null) p.LinkLeft._rawValue = _linkedBone.InverseMatrix * p.LinkLeft._rawValue;
        ...
        internal void Prepare()
        {
            ...
            foreach (var plane in _planes)
            {
                ...
                if (plane._linkLeft != null) plane._linkLeft._encodeIndex = -1;

                if (plane._linkRight != null) plane._linkRight._encodeIndex = -1;
            }
        }
        ...
        internal void Render()
        {
            ...
            foreach (var p in _planes) p.DrawPlanes(p);
        }
    ...
    public class CollisionLink
    {
        ...
        public Vector2 Value
        {
            get
            {
                if (_parent == null || _parent.LinkedBone == null) return _rawValue;
        ...
        {
            var count = _members.Count - 1;
            var links = new CollisionLink[count];
        ...
        {
            if (_parent != link._parent) return false;

            CollisionPlane plane;
            for (var i = link._members.Count; --i >= 0;)
            {
        ...
        public CollisionPlane Connect(CollisionLink p)
        {
            ...
            if (p == this || _parent != p._parent) return null;

            //Don't connect if plane already exists
            foreach (var plane in _members)
        ...
        {
            var link = new CollisionLink(_parent, point);
            var plane = new CollisionPlane(_parent, this, link);
            return link;
        }
        ...
        public void Delete()
        {
            while (_members.Count != 0) _members[0].Delete();
        }
        ...
        public void RemoveMember(CollisionPlane plane)
        {
            ...
            if (_members.Count == 0) _parent._points.Remove(this);
        }
    ...
    public unsafe class CollisionPlane
    {
            ...
            set
            {
                switch (_type = value)
                {
                    case CollisionPlaneType.Floor:
                        if (PointLeft._x > PointRight._x) SwapLinks();
                    ...
                    case CollisionPlaneType.RightWall:
                        if (PointLeft._y < PointRight._y) SwapLinks();
        ...
        public CollisionLink LinkLeft
        {
            ...
            set
            {
                if (_linkLeft != null) _linkLeft.RemoveMember(this);
        ...
        public CollisionLink LinkRight
        {
            ...
            set
            {
                if (_linkRight != null) _linkRight.RemoveMember(this);
        ...
        {
            var link = new CollisionLink(_parent, point);
            var plane = new CollisionPlane(_parent, link, _linkRight);
            LinkRight = link;
            return link;
        }
        ...
        internal void DrawPlanes(CollisionPlane p)
        {
            if (!_render) return;

            var clr = new Color4(1.0f, 0.0f, 1.0f, 1.0f);
            var l = _linkLeft.Value;
            var r = _linkRight.Value;

            var lev = 0;
            if (_linkLeft._highlight) lev++;

            if (_linkRight._highlight) lev++;

...
namespace BrawlLib.SSBB.ResourceNodes
{
    ...
    public unsafe class RELNode : ARCEntryNode, ModuleNode
    {
        ...
        public override bool OnInitialize()
        {
            ...
            for (var i = 0; i < Header->ImportListCount; i++)
            {
                var entry = &Header->Imports[i];
                uint id = entry->_moduleId;
                _imports.Add(id, new List<RELLink>());
                impNames[i] = _idNames.ContainsKey(id) ? _idNames[id] : $"Module{id}";

                var link = (RELLink*) (WorkingUncompressed.Address + entry->_offset);
                do
                {
            ...
            if (_name == null) _name = _idNames.ContainsKey(_id) ? _idNames[_id] : Path.GetFileName(_origPath);
        ...
        public override void OnPopulate()
        {
            ...
            var prevOffset = RELHeader.Size + RELSectionEntry.Size * (int) _numSections;
            var lastDataSection = -1;
            for (var i = 0; i < _numSections; i++)
            {
                var entry = Header->SectionInfo[i];
                var section = _sections[i] = new ModuleSectionNode();
                ...
                // Calculate buffer
                if (!Settings.Default.AutoCompressModules && i > 0 && dataOffset > 0)
                {
            ...
            if (lastDataSection != -1)
            {
                ...
                var entry = Header->SectionInfo[lastDataSection];
                int dataOffset = entry.Offset, dataSize = (int) (uint) entry._size;
            ...
            Action<object, DoWorkEventArgs> work = (sender, e) =>
            {
                ...
                foreach (var s in Sections)
                    ...
                    {
                        PPCOpCode code;
                        var opPtr = s.BufferAddress;
                        for (var i = 0; i < s._dataBuffer.Length / 4; i++)
                        ...
                        {
                            var target = x.Value.GetTargetRelocation();
                            string value = null;
            ...
            var bptr = (byte*) WorkingUncompressed.Address;
            var offset = findStageIDOffset();
            ...
            if (nodeContainsString("stOnlineTrainning"))
            {
                ...
                for (var i = 0; i < OTrainItemOffsets.Length; i++) _itemIDs[i] = bptr[OTrainItemOffsets[i]];
            }
        }

        public void ApplyRelocations()
        {
            foreach (var r in Sections) r._manager.ClearCommands();

            var offset = 0;
            var i = 0;
            foreach (var x in _imports.Keys)
            {
                var cmds = _imports[x];
                ModuleSectionNode section = null;
                foreach (var link in cmds)
                    ...
                    else
                    {
                        ...
                        if (link._type == RELLinkType.End || link._type == RELLinkType.IncrementOffset) continue;
        ...
        {
            _imports.Clear();
            var tempImports = new Dictionary<uint, ImportData>();
            foreach (var s in _sections)
            {
                foreach (var e in tempImports.Values)
                {
                    e._newSection = true;
                    e._lastOffset = 0;
                }
                ...
                foreach (var r in commands)
                {
                    ...
                    ImportData impData;
                    var moduleID = command._moduleID;
                    ...
                    var diff = offset - impData._lastOffset;
                    ...
                    var targetSection = (byte) command._targetSectionId;
                    var type = (RELLinkType) command._command;
                    var val = command._addend;
            ...
            foreach (var cmds in _imports.Values) cmds.Add(new RELLink {_type = RELLinkType.End});
        }
        ...
        public override int OnCalculateSize(bool force)
        {
            ...
            var size = RELHeader.Size + Children.Count * RELSectionEntry.Size +
            ...
            foreach (ModuleSectionNode s in Children)
            {
                ...
                var r = s.CalculateSize(true);
                if (!s._isBSSSection) size += r;
            }

            foreach (var s in _imports.Values) size += s.Count * RELLink.Size;
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            var sections = (RELSectionEntry*) (address + RELHeader.Size);
            var dataAddr = address + RELHeader.Size + Children.Count * RELSectionEntry.Size;
            foreach (ModuleSectionNode s in Children)
            ...
            var imports = (RELImportEntry*) dataAddr;
            ...
            var k = new List<uint>();
            foreach (var s in _imports.Keys)
                if (s != ModuleID && s != 0)
                    k.Add(s);
            ...
            {
                var id = k[i];
                var offset = (uint) (dataAddr - address);
                if (id == ModuleID) header->_commandOffset = offset;

                imports[i]._moduleId = id;
                imports[i]._offset = offset;

                var link = (RELLink*) dataAddr;
                foreach (var n in _imports[k[i]]) *link++ = n;

                dataAddr = link;
            }

            // Stage module conversion
            var bptr = (byte*) address;
            ...
            if (_itemIDs != null)
                ...
                {
                    var offset = OTrainItemOffsets[i];
        ...
        {
            var header = (RELHeader*) source.Address;
        ...
        {
            var b = new byte?[needle.Length];
            for (var i = 0; i < b.Length; i++) b[i] = needle[i];
        ...
        private int findStageIDOffset()
        {
            ...
            var index = arrayIndexOf(WorkingUncompressed.Address, WorkingUncompressed.Length, searchFor);
            return index < 0
                ? -1
                : index + 11;
        }

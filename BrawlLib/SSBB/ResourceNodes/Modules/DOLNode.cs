...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class DOLNode : ResourceNode, ModuleNode
    {
        ...
        {
            var s = v.StartsWith("0x")
                ? v.Substring(2, Math.Min(v.Length - 2, 8))
                : v.Substring(0, Math.Min(v.Length, 8));
            if (uint.TryParse(s, NumberStyles.HexNumber, null, out var t))
            {
                value = t;
                return true;
            }
        ...
        {
            var header = (DOLHeader*) address;
            var dataAddr = address + DOLHeader.Size;
            var offset = DOLHeader.Size;
            foreach (ModuleSectionNode s in Children)
            {
                var i = s.Index;
            ...
            for (var i = 0; i < 28; i++) header->padding[i] = 0;
        }
    }

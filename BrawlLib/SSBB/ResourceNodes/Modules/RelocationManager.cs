    ...
    public unsafe class RelocationManager
    {
        ...
        public void SetInt(int index, int value)
        {
            *((bint*) DataNode._dataBuffer.Address + index + _referenceIndex) = value;
            if (_data._linkedEditor != null) _data._linkedEditor.hexBox1.Invalidate();
        }
        ...
        {
            if (_reference != null) return _reference._manager.GetCommand(index + _referenceIndex);

            if (_commands.ContainsKey(index)) return _commands[index];
        ...
        {
            if (_commands[index] == null) return;

            var cmdTarget = _commands[index].GetTargetRelocation();
            ModuleSectionNode targetSection;
            if (cmdTarget != null && (targetSection = cmdTarget.Section) != null)
            {
                var thisRelocation = CreateTarget(index);
        ...
        {
            if (_reference != null) return _reference._manager.GetColor(index + _referenceIndex);

            if (_colors.ContainsKey(index)) return _colors[index];
        ...
        {
            if (_linkedCommands.ContainsKey(index)) return _linkedCommands[index];
        ...
        {
            if (_linkedBranches.ContainsKey(index)) return _linkedBranches[index];
        ...
        {
            if (_reference != null) return _reference._manager.GetTags(index + _referenceIndex);

            if (_tags.ContainsKey(index)) return _tags[index];
        ...
        {
            if (_reference != null) return _reference._manager.GetStatusColorFromIndex(_referenceIndex + index);

            if (GetCode(index) is PPCblr) return clrBlr;

            if (GetCode(index) is PPCBranch && !_commands.ContainsKey(index)) return clrBranch;

            if (!_commands.ContainsKey(index)) return clrNotRelocated;
        ...
        {
            if (_reference != null) return _reference._manager.GetTargetRelocation(index + _referenceIndex);

            if (_targetRelocations.ContainsKey(index)) return _targetRelocations[index];
    ...
    public class RelocationTarget
    {
        ...
        public override string ToString()
        {
            return string.Format("{0}[{1}] 0x{2}",
                RELNode._idNames.ContainsKey(_moduleID) ? RELNode._idNames[_moduleID] : "m" + _moduleID, _sectionID,
                (_index * 4).ToString("X"));
        }
        ...
        {
            if (obj is RelocationTarget) return obj.GetHashCode() == GetHashCode();

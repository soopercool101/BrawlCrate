...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class MDL0BoneNode : MDL0EntryNode, IBoneNode
    {
        ...
        [Browsable(false)] public MDL0BoneNode OverrideBone { get; set; }
        ...
        public void RecalcBindState(bool updateMesh, bool moveMeshWithBone, bool updateAssetLists = true)
        {
            ...
            else
            {
                ...
                var changed = new List<MDL0ObjectNode>();
                if (moveMeshWithBone || updateAssetLists) RecursiveGetInfluencedObjects(ref changed);
                ...
                else //Need to move vertices rigged to influences to the new position
                {
                    ...
                    foreach (var o in changed)
                        if (o._manager != null)
                        {
                            var pData = (Vector3*) o._manager._faceData[1].Address;
                            if (o.MatrixNode != null)
                            {
                                if (o.MatrixNode is Influence)
                                {
                                    var inf = (Influence) o.MatrixNode;
                                    foreach (var v in o._manager._vertices)
                                    {
                                        v._position *= inf.Matrix;
                                        foreach (var i in v._faceDataIndices)
                            ...
                            else
                            {
                                foreach (var v in o._manager._vertices)
                                    ...
                                    {
                                        var inf = (Influence) v.MatrixNode;
                                        v._position *= inf.Matrix;
                                        foreach (var i in v._faceDataIndices)
                                            pData[i] *= inf.Matrix.GetRotationMatrix();
                                    }
                    ...
                    RecursiveRecalcBindState(true, false);
                    foreach (var inf in Model._influences._influences) inf.CalcMatrix();
        ...
        {
            if (OverrideBone != null)
            {
                _frameMatrix = OverrideBone._frameMatrix;
                _inverseFrameMatrix = OverrideBone._inverseFrameMatrix;
            }
            else
            {
            ...
            foreach (MDL0BoneNode bone in Children) bone.RecalcFrameState(v);
        }

        [Browsable(false)] public List<Influence> LinkedInfluences { get; } = new List<Influence>();

        public MDL0BoneNode Clone()
        {
            var b = new MDL0BoneNode
            {
                _name = _name,
        ...
        internal override void GetStrings(StringTable table)
        {
            ...
            foreach (MDL0BoneNode n in Children) n.GetStrings(table);
        ...
        public override bool OnInitialize()
        {
            ...
            if (_billboardFlags != BillboardFlags.Off) Model._billboardBones.Add(this); //Update mesh in T-Pose
        ...
        {
            var header = (MDL0Bone*) WorkingUncompressed.Address;
        ...
        public void CalcFlags()
        {
            ...
            if (Scale._x == Scale._y && Scale._y == Scale._z) _boneFlags |= BoneFlags.ScaleEqual;

            if (Users.Count > 0) _boneFlags |= BoneFlags.HasGeometry;

            if (Scale == new Vector3(1)) _boneFlags |= BoneFlags.FixedScale;

            if (Rotation == new Vector3(0)) _boneFlags |= BoneFlags.FixedRotation;

            if (Translation == new Vector3(0)) _boneFlags |= BoneFlags.FixedTranslation;

            if (Parent is MDL0BoneNode)
            {
                if (BindMatrix == ((MDL0BoneNode) Parent).BindMatrix &&
                    InverseBindMatrix == ((MDL0BoneNode) Parent).InverseBindMatrix) _boneFlags |= BoneFlags.NoTransform;
            }
        ...
        {
            var header = (MDL0Bone*) dataAddress;
        ...
        {
            var m = influence ? InverseBindMatrix : BindMatrix;

            var rm = m.GetRotationMatrix();
            foreach (var user in Users)
                if (user is MDL0ObjectNode)
                {
                    Vector3* pData = null;
                    var o = user as MDL0ObjectNode;

                    if (o._manager._faceData[1] != null) pData = (Vector3*) o._manager._faceData[1].Address;
                ...
                {
                    var v = user as Vertex3;
                    var o = v.Parent as MDL0ObjectNode;
                    ...
                    {
                        var pData = (Vector3*) o._manager._faceData[1].Address;
                        foreach (var i in v._faceDataIndices) pData[i] *= rm;
                    }
                }

            foreach (var o in _singleBindObjects)
            {
                Vector3* pData = null;
                if (o._manager._faceData[1] != null) pData = (Vector3*) o._manager._faceData[1].Address;
        ...
        private void RecursiveGetInfluencedObjects(ref List<MDL0ObjectNode> changed)
        {
            ...
            foreach (MDL0BoneNode bone in Children) bone.RecursiveGetInfluencedObjects(ref changed);
        }

        private void RecursiveRecalcBindState(bool updateMesh, bool animPose)
        {
            if (!updateMesh) InfluenceAssets(false);
            ...
            if (!updateMesh) InfluenceAssets(true);

            foreach (MDL0BoneNode bone in Children) bone.RecursiveRecalcBindState(updateMesh, animPose);
        ...
        {
            if (BillboardSetting == BillboardFlags.Off) return;

            var camPoint = camera.GetPoint();
            var camRot = camera._rotation;

            var worldState = _frameMatrix.Derive();

            Matrix m = Matrix.Identity, mInv = Matrix.Identity;

            var rot = ((int) BillboardSetting & 1) == 0
                ? //If perspective
        ...
        {
            var box = bindBox ? _extents : GetBox();
        ...
        public Box GetBox()
        {
            if (_visDrawCalls.Count == 0) return new Box();

            var box = Box.ExpandableVolume;
            foreach (var o in _visDrawCalls) box.ExpandVolume(o._parentObject.GetBox());
        ...
        {
            _extents = GetBox();
            foreach (MDL0BoneNode b in Children) b.SetBox();
        }

        public List<MDL0BoneNode> ChildTree(List<MDL0BoneNode> list)
        {
            list.Add(this);
            foreach (MDL0BoneNode c in _children) c.ChildTree(list);
        ...
        public int BoneIndex
        {
            ...
            set
            {
                if (_entryIndex == value) return;

                var down = value < _entryIndex;
...
Y: Only the Y axis is allowed to rotate. Is affected by the parent bone's rotation.")]
        public BillboardFlags BillboardSetting
        {
            ...
            set
            {
                if (_billboardFlags == value) return;
        ...
        private void OnBillboardModeChanged()
        {
            if (BillboardSetting == BillboardFlags.Off)
            {
                ...
                {
                    BBRefNode = n;
                    foreach (MDL0BoneNode b in Children) b.RecursiveSetBillboard(BBRefNode);
                }
                else
                {
                    BBRefNode = null;
                    foreach (MDL0BoneNode b in Children) b.RecursiveSetBillboard(null);
                }
            }
            else
            {
                BBRefNode = null;
                foreach (MDL0BoneNode b in Children) b.RecursiveSetBillboard(this);
            }
        }
        ...
        private void RecursiveSetBillboard(MDL0BoneNode node)
        {
            ...
            {
                BBRefNode = node;
                foreach (MDL0BoneNode b in Children) b.RecursiveSetBillboard(node);
            }
        }
        ...
        internal void ApplyCHR0(CHR0Node node, float index)
        {
            ...
            foreach (MDL0BoneNode b in Children) b.ApplyCHR0(node, index);
        }
        ...
        public void Render(bool targetModel, ModelPanelViewport viewport, Vector3 parentPos = new Vector3())
        {
            ...
            {
                var screenPos = viewport.Camera.Project(_frameMatrix.GetPoint());
                viewport.ScreenText[Name] = new Vector3(screenPos._x, screenPos._y - 9.0f, screenPos._z);
            }

            var alpha = targetModel ? 1.0f : 0.45f;
            ...
            //Draw bone line
            var currentPos = _frameMatrix.GetPoint();
            ...
            var ignoreBoneScale = true;
            var scaleBones = viewport != null && viewport._renderAttrib._scaleBones;
            var transform = _frameMatrix;
            if (ignoreBoneScale)
            ...
            {
                if (scaleBones) transform.Scale(new Vector3(ModelEditorBase.OrbRadius(this, viewport.Camera)));
            ...
            //Render children
            foreach (MDL0BoneNode n in Children) n.Render(targetModel, viewport, currentPos);
        }

        public static GLDisplayList CreateNodeOrb()
        {
            var circle = TKContext.GetRingList();
            var orb = new GLDisplayList();

...
Those properties can use this color as an argument. This color is referred to as ";
        ...
        public MDL0ShaderNode ShaderNode
        {
            ...
            set
            {
                ...
                if (_shader != null) _shader._materials.Remove(this);
        ...
        public string Shader
        {
            ...
            set
            {
                ...
                {
                    var node = Model.FindChild(string.Format("Shaders/{0}", value), false) as MDL0ShaderNode;
                    if (node != null) ShaderNode = node;
                }
...
For example, if the shader has two stages but this number is 1, the second stage in the shader will be ignored.")]
        ...
        public sbyte LightSetIndex
        {
            ...
            set
            {
                if (!CheckIfMetal())
                {
                    ...
                    if (MetalMaterial != null) MetalMaterial.UpdateAsMetal();
        ...
        public void UpdateAsMetal()
        {
            ...
            if (MetalMaterial != null)
            {
                ...
                if (Children.Count - 1 != MetalMaterial.Children.Count)
                {
                    ...
                    //Start over
                    for (var i = 0; i <= MetalMaterial.Children.Count; i++)
                    {
        ...
        public MDL0MaterialNode MetalMaterial
        {
            get
            {
                foreach (MDL0MaterialNode t in Model._matList)
                    if (!IsMetal)
                    {
                        if (t.Name.StartsWith(Name) && t.IsMetal && Name.Length + 7 == t.Name.Length) return t;
                    }
        ...
        public override bool OnInitialize()
        {
            ...
            var mode = header->DisplayLists(_initVersion);
            ...
            var TexMatrices = header->TexMatrices(_initVersion);
            _texMtxFlags = TexMatrices->_mtxFlags;

            var Light = header->Light(_initVersion);
        ...
        {
            var first = Header->First;
            for (var i = 0; i < Header->_numTextures; i++)
        ...
        internal override void GetStrings(StringTable table)
        {
            ...
            foreach (MDL0MaterialRefNode n in Children) n.GetStrings(table);
        }
        ...
        public override int OnCalculateSize(bool force)
        {
            ...
            if ((size + _mdlOffset) % 0x20 != 0) size += size - 0x10 >= temp ? -0x10 : 0x10;
        ...
        {
            var model = Model;
            var header = (MDL0Material*) address;
            ...
            if (Model._isImport)
                ...
                for (var i = 0; i < Children.Count; i++, mtx += 3, i1++, i2++)
                {
                    var node = (MDL0MaterialRefNode) Children[i];
            ...
            var TexSettings = header->TexMatrices(model._version);
            *TexSettings = MDL0TexSRTData.Default;
            ...
            for (var i = Children.Count - 1; i >= 0; i--)
            {
                ...
                var flags = TexFlags.Enabled;

                //Check for non-default values
                if (node._bindState.Scale == new Vector2(1)) flags |= TexFlags.FixedScale;

                if (node._bindState.Rotate == 0) flags |= TexFlags.FixedRot;

                if (node._bindState.Translate == new Vector2(0)) flags |= TexFlags.FixedTrans;
            ...
            var Light = header->Light(model._version);
            ...
            //Rebuild references
            var mRefs = header->First;
            foreach (MDL0MaterialRefNode n in Children) n.Rebuild(mRefs++, MDL0TextureRef.Size, true);
            ...
            *header->IndMtxBlock(model._version) = _indMtx;
            ...
            var mode = header->DisplayLists(model._version);
            *mode = MatModeBlock.Default;
            ...
            i2 = 0x1050;
            mtx = 0;
            var xfData = (byte*) header->DisplayLists(model._version) + 0xE0;
        ...
        {
            var header = (MDL0Material*) dataAddress;
            ...
            var first = header->First;
            foreach (MDL0MaterialRefNode n in Children) n.PostProcess(mdlAddress, first++, stringTable);
        }
        ...
        public override void Export(string outPath)
        {
            ...
            GetStrings(table);
            var dataLen = CalculateSize(false);
        ...
        public void UseProgram(MDL0ObjectNode node, bool forceRemake = false)
        {
            ...
            var updateVert = string.IsNullOrEmpty(_vertexShaderSource) || forceRemake;
            var updateMatFrag = string.IsNullOrEmpty(_fragShaderSource) || forceRemake;
            var updateShaderFrag = ShaderNode != null && ShaderNode._fragShaderSource == null || forceRemake;
            var updateProgram = _programHandle <= 0 ||
                                updateVert || _vertexShaderHandle <= 0 ||
                                updateMatFrag || updateShaderFrag || _fragShaderHandle <= 0 ||
                                _internalForceRemake;

            if (updateShaderFrag)
                foreach (var m in ShaderNode.Materials)
            ...
            if (updateProgram)
            {
                ...
                try
                {
                    ...
                    if (_programHandle > 0)
                    {
                        if (_vertexShaderHandle > 0) DeleteShader(ref _vertexShaderHandle);

                        if (_fragShaderHandle > 0) DeleteShader(ref _fragShaderHandle);
                    ...
                    if (updateShaderFrag) ShaderNode._fragShaderSource = ShaderGenerator.GenTEVFragShader();

                    if (updateVert) _vertexShaderSource = ShaderGenerator.GenVertexShader();

                    if (updateMatFrag) _fragShaderSource = ShaderGenerator.GenMaterialFragShader();

                    var combineFrag = ShaderGenerator.CombineFragShader(
                        _fragShaderSource,
        ...
        internal override void Unbind()
        {
            ...
            foreach (MDL0MaterialRefNode m in Children) m.Unbind();
        }
        ...
        internal void ApplyCLR0(CLR0Node node, float index)
        {
            ...
            var mat = node.FindChild(Name, false) as CLR0MaterialNode;
            if (mat != null)
        ...
        internal void ApplySCN(SCN0Node node, float index)
        {
            ...
            else
            {
                ...
                var lightSetGroup = node.LightSetGroup;
                var lightGroup = node.LightGroup;
                var fogGroup = node.FogGroup;
                var cameraGroup = node.CameraGroup;

                if (lightSetGroup != null)
                {
                    var lightSet = LightSetIndex < lightSetGroup.Children.Count && LightSetIndex >= 0
                        ? lightSetGroup.Children[LightSetIndex] as SCN0LightSetNode
                        : null;
                    if (lightSet != null)
                    {
                        var ambLight = lightSet._ambient;
                        if (ambLight != null) _ambientLight = ambLight.GetAmbientColorFrame(index);

                        for (var i = 0; i < lightSet._lights.Length; i++)
                        {
                            var light = lightSet._lights[i];
                            if (light != null) _lights[i] = light.GetGLSLAnimFrame(index);
                        }
                    }
                }
                ...
                if (fogGroup != null)
                {
                    var fog = FogIndex < fogGroup.Children.Count && FogIndex >= 0
                        ? fogGroup.Children[FogIndex] as SCN0FogNode
                        : null;
                    if (fog != null) _fog = fog.GetAnimFrame(index);
                }
            }
        }
        ...
        public void ApplyViewportLighting(ModelPanelViewport viewport)
        {
            ...
            if (!_scn0Applied)
            {
                var pointLight = viewport._posLight._w != 0.0f;
    ...
    public class LightChannelControl
    {
        ...
        public bool Enabled
        {
            ...
            {
                _binary[1] = value;
                if (_parent != null) _parent._parent.SignalPropertyChange();
            }
        }

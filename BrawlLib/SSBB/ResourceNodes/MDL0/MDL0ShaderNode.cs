...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class MDL0ShaderNode : MDL0EntryNode
    {
        ...
        public void Default(bool change)
        {
            ...
            var stage = new MDL0TEVStageNode();
        ...
        {
            var grp = Header->First;
            for (var r = 0; r < 8; r++, grp = grp->Next)
                if (grp->_mask.Reg == 0x61)
                {
                    var s0 = new MDL0TEVStageNode();

                    var ksel = new KSel(grp->_ksel.Data.Value);
                    var tref = new RAS1_TRef(grp->_tref.Data.Value);
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            *header->SwapBlock = _swapBlock;
            ...
            var grp = (StageGroup*) (address + 0x80);
            for (var i = 0; i < Children.Count; i++)
            {
                var c = (MDL0TEVStageNode) Children[i]; //Current Stage
                ...
                {
                    var p = (MDL0TEVStageNode) Children[i - 1]; //Previous Stage

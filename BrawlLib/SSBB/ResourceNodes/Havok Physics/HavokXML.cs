    ...
    public class HavokXML
    {
        ...
        public static void Serialize(HavokNode node, string outFile)
        {
            ...
            using (var writer = XmlWriter.Create(stream, _writerSettings))
            {
                ...
                var classIDs = new Dictionary<HavokClassNode, int>();
                var i = 1;
                foreach (HavokSectionNode s in node.Children)
                foreach (var c in s._classCache)
                    if (!classIDs.ContainsKey(c))
                        classIDs.Add(c, i++);

                //RecursiveGetClassIDsData(ref classIDs, node, node._dataSection);

                var rootClass = node._dataSection.Children[0] as HavokClassNode;
                var sections =
                ...
                writer.WriteStartDocument();
                {
                    ...
                    writer.WriteAttributeString("predicates", "");
                    {
                        foreach (var s in sections)
                        {
                            ...
                            {
                                for (var x = s._classCache.Count - 1; x >= 0; x--)
                                {
                                    var c = s._classCache[x];
            ...
            ResourceNode e)
        {
            ...
            {
                var data = e as HavokClassNode;
                var type = node.GetClassNode(data._className);
                RecursiveGetClassIDsClasses(ref classIDs, node, type);
                classIDs.Add(data, classIDs.Count + 1);
            }

            for (var i = e.Children.Count - 1; i >= 0; i--) RecursiveGetClassIDsData(ref classIDs, node, e.Children[i]);
        }
            ...
            ResourceNode e)
        {
            if (e is hkClassNode)
            {
                var type = e as hkClassNode;
                if (!string.IsNullOrEmpty(type.ParentClass))
                {
                    var c = node.GetClassNode(type.ParentClass);
                    classIDs.Add(c, classIDs.Count + 1);
                }

                var members = type.FindChild("Members", false);
                foreach (hkClassMemberNode member in members.Children)
                {
                    if (!string.IsNullOrEmpty(member._class))
                    {
                        var c = node.GetClassNode(member._class);
                        classIDs.Add(c, classIDs.Count + 1);
                    }

                    if (!string.IsNullOrEmpty(member._enum))
                    {
                        var c = node.GetClassNode(member._enum);
                        classIDs.Add(c, classIDs.Count + 1);
                    }
                }

                if (!classIDs.ContainsKey(type)) classIDs.Add(type, classIDs.Count + 1);
            }
        }

        public static string SwapName(string className)
        {
            if (HardcodedName.ContainsKey(className)) return HardcodedName[className].Key;
        ...
        {
            if (HardcodedName.ContainsKey(className)) return HardcodedName[className].Value;

...
namespace BrawlLib.SSBB.ResourceNodes
{
    ...
    public class HavokMetaObjectNode : ClassMemberInstanceNode
    {
        ...
        public override bool OnInitialize()
        {
            ...
            if (node._allSignatures.ContainsKey(_className)) _signature = HavokNode._allSignatures[_className];

            node._dataSection._classCache.Add(this);

            if (_name == null) _name = _classNode.Name;

            _memberArray = new List<hkClassMemberNode>();

            //Youngest class has size for all inherited classes included
            if (_classNode._inheritance.Count > 0) SetSizeInternal(_classNode._inheritance[0].Size);

            foreach (var c in _classNode._inheritance)
            {
                var members = c.FindChild("Members", false);
                if (members != null) _memberArray.AddRange(members.Children.Select(x => x as hkClassMemberNode));
            }
        ...
        public override void OnPopulate()
        {
            ...
            foreach (var member in _memberArray)
            {
                var type = member.Type;
                var zero = type == hkClassMember.Type.TYPE_ZERO;
                if (zero) type = member.SubType;
                ...
                {
                    var c = HavokNode.GetClassNode(member._class);
                    if (c is hkClassNode) memberClass = c as hkClassNode;

                    if (memberClass == null) Console.WriteLine("Could not find " + member._class + " class");
                }
                ...
                if (!string.IsNullOrEmpty(member._enum))
                {
                    ...
                    foreach (var c in _classNode._inheritance)
                    {
                        var enums = c.FindChild("Enums", false);
                        if (enums != null)
                ...
                var instance = TryGetMember(type);
                if (instance != null)
                {
                    switch (type)
                    {
        ...
        public static ClassMemberInstanceNode TryGetMember(hkClassMember.Type type)
        {
            ...
            {
                var t = MemberTypes[index];
                if (t != null) m = Activator.CreateInstance(t) as ClassMemberInstanceNode;
            }

            if (m == null) Console.WriteLine("Problem creating class member instance of " + type);

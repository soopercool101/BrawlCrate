...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class SCN0LightNode : SCN0EntryNode, IBoolArraySource, IColorSource, IKeyframeSource
    {
        ...
        internal void SetSize(int numFrames, bool looped)
        {
            ...
            var numBytes = Math.Min(_entryCount.Align(32) / 8, _data.Length);
        ...
        public override int OnCalculateSize(bool force)
        {
            ...
            if (_name == "<null>") return SCN0Light.Size;

            for (var i = 0; i < Keyframes.ArrayCount; i++) CalcKeyLen(Keyframes[i]);

            for (var i = 0; i < 2; i++)
            {
                if (i == 1 && !SpecularEnabled) break;

                _matches[i] = FindColorMatch(_constants[i], Scene.FrameCount, i) as SCN0LightNode;
                if (_matches[i] == null && !_constants[i]) _dataLengths[1] += 4 * (FrameCount + 1);
            }

            if (!ConstantVisibility) _dataLengths[2] += _entryCount.Align(32) / 8;

            //If this light uses specular lighting, 
            //increment SCN0 specular light count
            if (SpecularEnabled && Scene != null) Scene._specLights++;
        ...
        public bool[] _constants = {true, true};
        ...
        public int NonSpecularLightID
        {
            get
            {
                if (!SpecularEnabled) return 0;

                var i = 0;
                foreach (SCN0LightNode n in Parent.Children)
                {
                    if (n.Index == Index) return Parent.Children.Count + i;

                    if (n.SpecularEnabled && n.Index != Index) i++;
                }
        ...
        private static readonly FixedFlags[] _ordered =
        {
        ...
        internal void SetNumEntries(int id, int value)
        {
            ...
            {
                var p = _numEntries[id] > 0
                    ? (ARGBPixel) GetColors(id)[_numEntries[id] - 1]
                    : new ARGBPixel(255, 0, 0, 0);
                for (var i = value - _numEntries[id]; i-- > 0;) GetColors(id).Add(p);
            }
        ...
        {
            if (_data.Length == 0) return Enabled;

            var i = index >> 3;
            var bit = 1 << (7 - (index & 0x7));
        ...
        {
            var enabled = GetEnabled(index);

            //No point calculating anything if the light isn't used
            if (!enabled) return new GLSLLightFrame();
        ...
        public LightAnimationFrame GetAnimFrame(float index)
        {
            ...
            for (var x = 0; x < Keyframes.ArrayCount; x++)
            {
                var a = Keyframes[x];
            ...
            {
                var colorIndex = ((int) Math.Truncate(index)).Clamp(0, _lightColor.Count - 1);
            ...
            if (SpecularEnabled)
            {
                ...
                {
                    var specIndex = ((int) Math.Truncate(index)).Clamp(0, _specColor.Count - 1);
        ...
        public static Vector3 GetLightSpotCoefs(float cutoff, SpotFn spotFunc)
        {
            ...
            if (cutoff <= 0.0f || cutoff > 90.0f) spotFunc = SpotFn.Off;
        ...
        public static Vector3 GetLightDistCoefs(float distance, float brightness, DistAttnFn distFunc)
        {
            ...
            if (distance < 0.0F || brightness <= 0.0F || brightness >= 1.0F) distFunc = DistAttnFn.Off;

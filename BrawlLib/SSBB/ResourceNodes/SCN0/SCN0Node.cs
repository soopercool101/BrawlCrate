...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class SCN0Node : NW4RAnimationNode
    {
        ...
        public override void OnPopulate()
        {
            ...
            {
                var name = group->First[i].GetName();
                (g = new SCN0GroupNode(name)).Initialize(this, new DataSource(group->First[i].DataAddress, 0));

                var typeID = (int) g._type;
                _groups[typeID] = g;

                var t = SCN0GroupNode._types[typeID];

                ResourceNode r;
                var entryAddr = (VoidPtr) Header + offsets[typeID];
        ...
        {
            var typeID = (int) type;
            if (typeID < 0 || typeID >= 5) return null;

            if (_groups[typeID] == null) AddChild(_groups[typeID] = new SCN0GroupNode(type));
        ...
        {
            table.Add(Name);
            foreach (SCN0GroupNode n in Children) n.GetStrings(table);
        ...
        public override int OnCalculateSize(bool force)
        {
            ...
            var size = SCN0v4.Size + ResourceGroup.Size + Children.Count * ResourceEntry.Size;

            //Add the size of each resource group, and headers and all data
            foreach (SCN0GroupNode n in Children) size += n.CalculateSize(true);
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            *group = new ResourceGroup(Children.Count);
            ...
            for (var i = 0; i < 5; i++)
            {
                ...
                var g = indices[i] >= 0 ? Children[indices[i]] as SCN0GroupNode : null;
                if (g != null)
                {
                    ...
                    for (var x = 0; x < 4; x++) g._addrs[x] = dataAddrs[x + 1];
                    ...
                    for (var x = 0; x < 5; x++) dataAddrs[x] += g._dataLengths[x];
                }
            }
            ...
            if (_version == 5)
            {
                ...
                if (_userEntries.Count > 0) _userEntries.Write(header->UserData = dataAddrs[4]);
            }
            else
            {
                var header = (SCN0v4*) address;
            ...
            StringTable stringTable)
        {
            ...
            var index = 1;
            int[] indices = {-1, -1, -1, -1, -1};
            foreach (SCN0GroupNode g in Children) indices[(int) g._type] = g.Index;

            var addr = dataAddress;
            for (var i = 0; i < 5; i++)
            {
                var n = indices[i] >= 0 ? Children[indices[i]] as SCN0GroupNode : null;
                if (n != null)
                {
            ...
            if (_version == 5) _userEntries.PostProcess(((SCN0v5*) addr)->UserData, stringTable);
        }
        ...
        {
            var group = GetOrCreateFolder(type);
            if (group == null) return null;

            var t = SCN0GroupNode._types[(int) type];
            var n = Activator.CreateInstance(t) as SCN0EntryNode;
            n.Name = group.FindName(name);
            group.AddChild(n);

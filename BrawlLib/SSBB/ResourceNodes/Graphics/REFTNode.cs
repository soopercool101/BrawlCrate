...
namespace BrawlLib.SSBB.ResourceNodes
{
    ...
    public unsafe class REFTEntryNode : ResourceNode, IImageSource, IColorSource
    {
        ...
        [Category("REFT Image")] public WiiPixelFormat TextureFormat { get; private set; }

        [Category("REFT Image")] public WiiPaletteFormat PaletteFormat { get; private set; }

        [Category("REFT Image")] public int Colors { get; private set; }

        [Category("REFT Image")] public int Width { get; private set; }

        [Category("REFT Image")] public int Height { get; private set; }

        [Category("REFT Image")] public int ImageLength { get; private set; }

        [Category("REFT Image")] public int PaletteLength { get; private set; }

        [Category("REFT Image")] public int LevelOfDetail { get; private set; }
        ...
        {
            get => HasPlt
                ? _palette ?? (_palette = TextureConverter.DecodePalette((byte*) Header + 0x20 + Header->_imagelen,
                      Colors, PaletteFormat))
                : null;
            set
            {
                _palette = value;
            ...
            try
            {
                if (HasPlt)
                    return TextureConverter.DecodeIndexed((byte*) Header + 0x20, Width, Height, Palette, index + 1,
                        TextureFormat);
                return TextureConverter.Decode((byte*) Header + 0x20, Width, Height, index + 1, TextureFormat);
            }
            catch
            {
                return null;
        ...
        public override bool OnInitialize()
        {
            ...
            TextureFormat = (WiiPixelFormat) Header->_format;
            PaletteFormat = (WiiPaletteFormat) Header->_pltFormat;
            Colors = Header->_colorCount;
            ImageLength = (int) Header->_imagelen;
            Width = Header->_width;
            Height = Header->_height;
            PaletteLength = (int) Header->_pltSize;
            LevelOfDetail = Header->_mipmap + 1;
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            var header = (REFTImageHeader*) address;
        ...
        public void Replace(Bitmap bmp)
        {
            FileMap tMap;
            if (HasPlt)
                tMap = TextureConverter.Get(TextureFormat).EncodeREFTTextureIndexed(bmp, LevelOfDetail,
                    Palette.Entries.Length, PaletteFormat, QuantizationAlgorithm.MedianCut);
            else
                tMap = TextureConverter.Get(TextureFormat).EncodeREFTTexture(bmp, LevelOfDetail, WiiPaletteFormat.IA8);

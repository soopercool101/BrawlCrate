...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class REFFParticleNode : ResourceNode
    {
        ...
        public override bool OnInitialize()
        {
            ...
            var addr = Params->paramDesc.textureNames.Address;
            for (var i = 0; i < 3; i++)
            {
                if (*(bushort*) addr > 1)
        ...
        {
            var size = 0x8C;
            foreach (var s in _textureNames)
            {
                size += 3;
                if (s != null && s.Length > 0) size += s.Length;
            }
        ...
        {
            var p = (ParticleParameterHeader*) address;
            p->headersize = (uint) length - 4;
            p->paramDesc = desc;
            var ptr = (sbyte*) p->paramDesc.textureNames.Address;
            foreach (var s in _textureNames)

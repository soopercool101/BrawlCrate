...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class REFFEmitterNode7 : ResourceNode
    {
        ...
        public override void OnPopulate()
        {
            var col1 = 0;
            var colop1 = col1 + 16;
            var alpha1 = colop1 + 20;
            var alphaop1 = alpha1 + 16;
            var csel1 = alphaop1 + 20;
            for (var i = 0; i < 4; i++)
            {
                var s = new REFFTEVStage(i);

                var p = (byte*) _drawSetting.mTevColor1.Address;
                {
                    s.kcsel = p[csel1 + i];
                    s.kasel = p[csel1 + 4 + i];
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            *(EmitterDrawSetting7*) &hdr->_drawSetting = _drawSetting;
            ...
            for (var i = 0; i < 4; i++)
            {
                var s = (REFFTEVStage) Children[i];

                var p = (byte*) (*(EmitterDrawSetting7*) &hdr->_drawSetting).mTevColor1.Address;
                {
        ...
        public ReffType ParticleType
        {
            ...
            set
            {
                if (!(ParticleType >= ReffType.Stripe && value >= ReffType.Stripe)) typeOption2._data = 0;
        ...
        public string StripeTubeVertexCount
        {
            ...
            set
            {
                if (ParticleType >= ReffType.Stripe && !string.IsNullOrEmpty(value))
                {
                    byte.TryParse(value, out var b);
                    if (b >= 3) _drawSetting.typeOption0 = b;
        ...
        public string StripeInterpDivisionCount
        {
            ...
            set
            {
                ...
                {
                    byte.TryParse(value, out var b);
                    if (b >= 1) _drawSetting.typeOption1 = b;

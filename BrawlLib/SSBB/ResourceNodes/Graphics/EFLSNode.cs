...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class EFLSNode : ARCEntryNode
    {
        ...
        public override int OnCalculateSize(bool force)
        {
            ...
            foreach (EFLSEntryNode e in Children)
            {
                ...
                if (e.Children.Count > 0)
                {
                    ...
                    re3dSize += 0x10;
                    foreach (RE3DEntryNode r in e.Children) re3dSize += 0x10 + r.Name.Length + 1 + r.Effect.Length + 1;
                }
            }

            if (re3dSize > 0) RE3DOffset = size.Align(0x10);
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            var header = (EFLSHeader*) address;
            var RE3DAddr = (RE3D*) ((VoidPtr) header + RE3DOffset);
            *header = new EFLSHeader(count, _brresCount, _unk1, _unk2);

            var entry = (EFLSEntry*) ((int) header + 0x10);
            var dPtr = (sbyte*) entry + count * 0x10;
            foreach (EFLSEntryNode n in Children)
            {
                var offset = n._name.Equals("<null>", StringComparison.OrdinalIgnoreCase)
                    ? 0
                    : (int) dPtr - (int) header;
                *entry = new EFLSEntry(n._brresID1, n._brresID2, offset, n._unk);

                if (offset > 0) n._name.Write(ref dPtr);
                ...
                if (n.Children.Count > 0)
                {
                    ...
                    var rEntry = (RE3DEntry*) ((VoidPtr) RE3DAddr + 0x10);
                    var sPtr = (sbyte*) rEntry + n.Children.Count * 0x10;
    ...
    public unsafe class EFLSEntryNode : ResourceNode
    {
        ...
        public uint BrresId
        {
            ...
            {
                _brresID2 = (int) value;
                if (UseBrres) _brresID1 = _brresID2;
                SignalPropertyChange();
            }
        }
        ...
        public override void OnPopulate()
        {
            var data = (RE3D*) ((VoidPtr) ((EFLSNode) Parent).Header + _re3dOffset);
            for (var i = 0; i < data->_numEntries; i++)

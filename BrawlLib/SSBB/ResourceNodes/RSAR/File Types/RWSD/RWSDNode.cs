...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class RWSDNode : RSARFileNode
    {
        ...
        {
            var group0 = new RWSDDataGroupNode
            {
                Parent = this
            };
            var group1 = new RWSDSoundGroupNode
            {
                Parent = this
            };
        }
        ...
        {
            foreach (RWSDDataNode node in Children[0].Children) builder.Add((uint) node.Index, node._name);
        }
        ...
        private void ParseBlocks()
        {
            ...
            var total = WorkingUncompressed.Length;

            SetSizeInternal(len);

            //Look for labl block
            var labl = (LABLHeader*) (dataAddr + len);
            ...
            //Set data source
            if (total > len) _audioSource = new DataSource(dataAddr + len, total - len);
        }
        ...
        public override void OnPopulate()
        {
            ...
            {
                //Get them from RSAR
                var symb2 = parent.Header->SYMBBlock;
                var info = parent.Header->INFOBlock;

                VoidPtr offset = &info->_collection;
                var soundList = info->Sounds;
            ...
            {
                var name = "Entry" + i;
                if (_labels != null && i < _labels.Length) name = _labels[i].String;

                var entry = (RWSD_DATAEntry*) list->Get(list, i);
                var node = new RWSDDataNode {_name = name};
                node._offset = list;
        ...
        public override int OnCalculateSize(bool force)
        {
            ...
            {
                foreach (var g in Children) _headerLen += g.CalculateSize(true);

                foreach (WAVESoundNode s in Children[1].Children) _audioLen += s._streamBuffer.Length;
            }
        ...
        {
            var addr = address + 0x20;

            var header = (RWSDHeader*) address;
            ...
            else
            {
                ...
                var audio = addr;
                if (RSARNode != null) audio = _rebuildAudioAddr;

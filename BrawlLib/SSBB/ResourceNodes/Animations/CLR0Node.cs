...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class CLR0Node : NW4RAnimationNode
    {
        ...
        {
            var node = new CLR0MaterialNode();
            var entry = new CLR0MaterialEntryNode
            {
        ...
        {
            var size = (_version == 4 ? CLR0v4.Size : CLR0v3.Size) + 0x18 + Children.Count * 0x10;
            foreach (CLR0MaterialNode n in Children)
            {
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            var pMat = (CLR0Material*) (address + (_version == 4 ? CLR0v4.Size : CLR0v3.Size) + 0x18 + count * 0x10);

            var offset = Children.Count * 8;
            foreach (CLR0MaterialNode n in Children) offset += n.Children.Count * 8;

            var pData = (RGBAPixel*) ((VoidPtr) pMat + offset);

            ResourceGroup* group;
            if (_version == 4)
            {
                var header = (CLR0v4*) address;
            ...
            *group = new ResourceGroup(count);
            ...
            foreach (CLR0MaterialNode n in Children)
            {
                ...
                var pMatEntry = (CLR0MaterialEntry*) ((VoidPtr) pMat + 8);
                foreach (CLR0MaterialEntryNode e in n.Children)
                {
                    ...
                    else
                    {
                        *pMatEntry = new CLR0MaterialEntry(e._colorMask, (int) pData - (int) ((VoidPtr) pMatEntry + 4));
                        foreach (var p in e._colors) *pData++ = p;
                    }
        ...
        {
            table.Add(Name);
            foreach (CLR0MaterialNode n in Children) table.Add(n.Name);
    ...
    public unsafe class CLR0MaterialNode : ResourceNode
    {
        ...
        {
            var header = (CLR0Material*) dataAddress;
            header->ResourceStringAddress = stringTable[Name] + 4;
        }
    ...
    public unsafe class CLR0MaterialEntryNode : ResourceNode, IColorSource
    {
        ...
        internal int NumEntries
        {
            ...
            set
            {
                ...
                {
                    var p = _numEntries > 0 ? _colors[_numEntries - 1] : new ARGBPixel(255, 0, 0, 0);
                    for (var i = value - _numEntries; i-- > 0;) _colors.Add(p);
                }
        ...
        public override bool OnInitialize()
        {
            ...
            if (_replaced && WorkingUncompressed.Length >= 16 && Header->_data == 8)
            {
                ...
                {
                    var frameCount = ((CLR0Node) Parent.Parent)._numFrames;
            ...
            else
            {
                ...
                else
                {
                    ...
                    var data = Header->Data;
                    for (var i = 0; i < _numEntries; i++) _colors.Add(*data++);
                }
            }
        ...
        {
            var length = 12 + (_numEntries != 0 ? _colors.Count * 4 : 4);
                ...
                FileOptions.RandomAccess))
            {
                ...
                {
                    var entry = (CLR0MaterialEntry*) map.Address;

                    entry->_colorMask = _colorMask;
                    entry->_data = 8;
                    *((bint*) entry + 2) = _numEntries;

                    var pData = entry->Data;
                    if (_numEntries != 0)
                        foreach (var p in _colors)
                            *pData++ = p;
                    else
                        *pData = _solidColor;
                }
        ...
        {
            _constant = false;
            var entries = ((CLR0Node) Parent._parent)._numFrames;

...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class PAT0Node : NW4RAnimationNode
    {
        ...
        public void RegenerateTextureList()
        {
            ...
            foreach (PAT0TextureEntryNode e in t.Children)
                ...
                {
                    if ((index = _textureFiles.IndexOf(e._tex)) < 0) _textureFiles.Add(e._tex);
                }
                else
                {
                    e._texFileIndex = 0;
        ...
        public void RegeneratePaletteList()
        {
            ...
            foreach (PAT0TextureEntryNode e in t.Children)
                ...
                {
                    if ((index = _paletteFiles.IndexOf(e._plt)) < 0) _paletteFiles.Add(e._plt);
                }
                else
                {
                    e._pltFileIndex = 0;
        ...
        public override bool OnInitialize()
        {
            ...
            if (_version == 4)
            {
                ...
                //Get texture strings
                for (var i = 0; i < texPtr; i++) _textureFiles.Add(header->GetTexStringEntry(i));

                //Get palette strings
                for (var i = 0; i < pltPtr; i++) _paletteFiles.Add(header->GetPltStringEntry(i));
        ...
        public override int OnCalculateSize(bool force)
        {
            ...
            var size = PAT0v3.Size + 0x18 + Children.Count * 0x10;
            size += (_textureFiles.Count + _paletteFiles.Count) * 8;
            foreach (PAT0EntryNode n in Children) size += n.CalculateSize(true);
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            var commonHeader = (PAT0v3*) address;
            ...
            //Set pointer offsets
            var offset = length - _textureFiles.Count * 4 - _paletteFiles.Count * 4;
            commonHeader->_texPtrTableOffset = offset;
            commonHeader->_pltPtrTableOffset = offset + _textureFiles.Count * 4;

            //Set pointers
            var ptr = (bint*) (commonHeader->Address + commonHeader->_texPtrTableOffset);
            for (var i = 0; i < _textureFiles.Count; i++) *ptr++ = 0;

            ptr = (bint*) (commonHeader->Address + commonHeader->_pltPtrTableOffset);
            for (var i = 0; i < _paletteFiles.Count; i++) *ptr++ = 0;

            var group = commonHeader->Group;
            *group = new ResourceGroup(Children.Count);
            ...
            foreach (PAT0EntryNode n in Children) dataAddress += n._entryLen;

            foreach (PAT0EntryNode n in Children)
            foreach (PAT0TextureNode t in n.Children)
            {
                n._dataAddrs[t.Index] = dataAddress;
                if (n._dataLens[t.Index] != -1) dataAddress += n._dataLens[t.Index];
            }
        ...
        public void Append(PAT0Node external)
        {
            ...
            foreach (PAT0TextureNode _extEntry in w.Children)
            {
                ...
                if ((_intEntry = (PAT0TextureNode) FindChild(w.Name + "/" + _extEntry.Name, false)) == null)
                {
                    ...
                    var newIntEntry = new PAT0TextureNode(_extEntry._texFlags, _extEntry.TextureIndex);

                    wi.AddChild(newIntEntry);
                    foreach (PAT0TextureEntryNode e in _extEntry.Children)
                    {
                        var q = new PAT0TextureEntryNode {_frame = e._frame + origIntCount};
    ...
    public unsafe class PAT0EntryNode : ResourceNode
    {
        ...
        public override bool OnInitialize()
        {
            ...
            for (var i = 0; i < 8; i++) texFlags[i] = (PAT0Flags) ((flags >> (i * 4)) & 0xF);
        ...
        public override void OnPopulate()
        {
            ...
            foreach (var p in texFlags)
            {
                if (p.HasFlag(PAT0Flags.Enabled))
                {
                    ...
                    {
                        var t = new PAT0TextureNode(p, index) {textureCount = 1};
                        t._parent = this;
                        _children.Add(t);
        ...
        public override int OnCalculateSize(bool force)
        {
            ...
            foreach (PAT0TextureNode table in Children) _dataLens[table.Index] = table.CalculateSize(true);

            //Check to see if any children can be remapped.
            foreach (PAT0TextureNode table in Children) table.CompareToAll();

            var size = 0;
            foreach (var i in _dataLens)
                if (i != -1)
                    size += i;
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            var header = (PAT0Pattern*) address;
            ...
            foreach (PAT0TextureNode table in Children)
            {
                ...
                {
                    var entry = (PAT0TextureEntryNode) table.Children[0];
                    var node = (PAT0Node) Parent;
        ...
        {
            var header = (PAT0Pattern*) dataAddress;
            header->ResourceStringAddress = stringTable[Name] + 4;
        }
        ...
        public void CreateEntry()
        {
            ...
            var node = new PAT0TextureNode((PAT0Flags) 7, value);
            AddChild(node);
            node.CreateEntry();
        }
    }
    ...
    public unsafe class PAT0TextureNode : ResourceNode, IVideo
    {
        ...
        public PAT0TextureEntryNode GetPrevious(float index)
        {
            ...
            foreach (PAT0TextureEntryNode next in Children)
            {
                ...
                if (prev._frame <= index && next._frame > index) break;
        ...
        public override void OnPopulate()
        {
            ...
            {
                var current = Header->Textures;
                for (var i = 0; i < textureCount; i++, current++)
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            if (Children.Count > 1)
            {
                var table = (PAT0TextureTable*) address;
                table->_textureCount = (short) Children.Count;
                var f = (Children[Children.Count - 1] as PAT0TextureEntryNode)._frame;
                table->_frameScale = f == 0 ? 0 : 1.0f / f;
                table->_pad = 0;

                var entry = table->Textures;
                foreach (PAT0TextureEntryNode n in Children) n.Rebuild(entry++, PAT0Texture.Size, true);
            }
        }
        ...
        internal void CompareToAll()
        {
            ...
            foreach (PAT0TextureNode table in e.Children)
            {
                ...
                {
                    var same = true;
                    for (var l = 0; l < Children.Count; l++)
                    {
                        var exte = (PAT0TextureEntryNode) table.Children[l];
                        var inte = (PAT0TextureEntryNode) Children[l];
        ...
        public void CreateEntry()
        {
            var value = Children.Count > 0 ? ((PAT0TextureEntryNode) Children[Children.Count - 1])._frame + 1 : 0;
            var node = new PAT0TextureEntryNode {_frame = value};
            AddChild(node);
        ...
        public void SortChildren()
        {
            ...
            {
                var t = Children[i] as PAT0TextureEntryNode;
                var x = t.Index;
                t.CheckNext();
                t.CheckPrev();
                if (t.Index != x) goto Top;
            }
        }
    ...
    public unsafe class PAT0TextureEntryNode : ResourceNode, IImageSource
    {
        ...
        public string Texture
        {
            ...
            set
            {
                if (!(Parent as PAT0TextureNode)._hasTex || value == _tex) return;
        ...
        public string Palette
        {
            ...
            set
            {
                if (!(Parent as PAT0TextureNode)._hasPlt || value == _plt) return;
        ...
        public Bitmap GetImage(int index)
        {
            if (string.IsNullOrEmpty(_tex)) return null;

            if (_textureNode == null)
            {
                _textureNode = RootNode.FindChildByType(_tex, true, ResourceType.TEX0) as TEX0Node;
                if (_textureNode == null) return null;
            }
        ...
        public void GetStrings()
        {
            ...
            var node = (PAT0Node) Parent.Parent.Parent;
            ...
            if (_name == null && _plt != null) _name = _plt;
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            var header = (PAT0Texture*) address;

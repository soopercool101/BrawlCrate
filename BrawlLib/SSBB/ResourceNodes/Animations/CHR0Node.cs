...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class CHR0Node : NW4RAnimationNode
    {
        ...
        {
            foreach (CHR0EntryNode n in Children) n.SetSize(_numFrames, Loop);
        }
        ...
        {
            FrameCount++;
            foreach (CHR0EntryNode c in Children) c.Keyframes.Insert(index, 0, 1, 2, 3, 4, 5, 6, 7, 8);
        }

        public void DeleteKeyframe(int index)
        {
            foreach (CHR0EntryNode c in Children) c.Keyframes.Delete(index, 0, 1, 2, 3, 4, 5, 6, 7, 8);
        ...
        {
            table.Add(Name);
            foreach (CHR0EntryNode n in Children) table.Add(n.Name);
        ...
        {
            var size = (_version == 5 ? CHR0v5.Size : CHR0v4_3.Size) + 0x18 + Children.Count * 0x10;
            foreach (CHR0EntryNode n in Children) size += n.CalculateSize(true);
        ...
        public static CHR0Node FromFile(string path)
        {
            ...
            try
            {
                if (path.EndsWith(".anim", StringComparison.OrdinalIgnoreCase)) return AnimFormat.Read(path);

                if (path.EndsWith(".txt", StringComparison.OrdinalIgnoreCase)) return CHR0TextImporter.Convert(path);

                if (path.EndsWith(".json", StringComparison.OrdinalIgnoreCase)) return CHR0JsonImporter.Convert(path);
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            *group = new ResourceGroup(Children.Count);
            ...
            foreach (CHR0EntryNode n in Children) dataAddress += n._entryLen;
            ...
            StringTable stringTable)
        {
            ...
            {
                var header = (CHR0v4_3*) dataAddress;
                header->ResourceStringAddress = (int) stringTable[Name] + 4;
                if (!string.IsNullOrEmpty(_originalPath)) header->OrigPathAddress = stringTable[_originalPath] + 4;
        ...
        public void Resize(int newFrameCount)
        {
            ...
            {
                var newCollection = new KeyframeCollection(9, newFrameCount + (Loop ? 1 : 0), 1, 1, 1);
        ...
        public void MergeWith(CHR0Node external)
        {
            ...
            if (external.FrameCount > FrameCount) FrameCount = external.FrameCount;
            ...
            foreach (CHR0EntryNode _extTarget in external.Children)
            {
                ...
                var entry = new CHR0EntryNode {Name = _extTarget.Name};
                entry.SetSize(_extTarget.FrameCount, Loop);

                //Apply all external keyframes to current entry.
                for (var x = 0; x < _extTarget.FrameCount; x++)
                for (var i = 0; i < 9; i++)
                ...
                else
                {
                    ...
                    else if (result == DialogResult.No)
                    {
                        ...
                        if (result == DialogResult.Yes)
                        {
                            ...
                            if (_extTarget.FrameCount > node.FrameCount) node.SetSize(_extTarget.FrameCount, Loop);
                                ...
                                if ((kfe = _extTarget.GetKeyframe(i, x)) != null)
                                {
                                    ...
                                    else
                                    {
                                        result = MessageBox.Show(null,
                                            ...
                                            "Replace Keyframe?", MessageBoxButtons.YesNoCancel);
                                        if (result == DialogResult.Yes)
                                        {
                                            node.SetKeyframe(i, x, kfe._value);
                                        }
        ...
        public void Append(CHR0Node external)
        {
            ...
            var extCount = external.FrameCount;
            FrameCount += extCount;
            ...
            foreach (CHR0EntryNode extEntry in external.Children)
            {
                ...
                {
                    var newIntEntry = new CHR0EntryNode {Name = extEntry.Name};
        ...
        private void FixChildren(MDL0BoneNode node, int axis)
        {
            ...
            {
                var _target = (CHR0EntryNode) FindChild(b.Name, true);
                if (_target != null)
                    switch (axis)
                    {
                        case 0: //X, correct Y and Z
                            for (var l = 0; l < _target.FrameCount; l++)
                            for (var g = 3; g < 6; g++)
                                if (g != 3)
        ...
        {
            var w = FindChild(boneName, false) as CHR0EntryNode;
            if (w == null) return;
    ...
    public unsafe class CHR0EntryNode : ResourceNode, IKeyframeSource
    {
        ...
        {
            var header = (CHR0Entry*) dataAddress;
        ...
        public KeyframeEntry SetKeyframe(int arrayIndex, int index, float value, bool forceNoGenTans = false,
            bool parsing = false)
        {
            var k = Keyframes.SetFrameValue(arrayIndex, index, value, parsing);

            if (!forceNoGenTans)
            {
                var exists = Keyframes.GetKeyframe(arrayIndex, index) != null;
                if (_generateTangents || !exists) k.GenerateTangent();

...
namespace BrawlLib.SSBB.ResourceNodes
{
    public unsafe class SHP0Node : NW4RAnimationNode
    {
        ...
        internal override void GetStrings(StringTable table)
        {
            ...
            foreach (var s in _strings) table.Add(s);
        ...
        public override bool OnInitialize()
        {
            ...
            if (_version == 4)
            {
                ...
                var stringOffset = header->StringEntries;
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            *group = new ResourceGroup(Children.Count);
            ...
            foreach (SHP0EntryNode n in Children) dataAddress += n._entryLen;
        ...
        public override int OnCalculateSize(bool force)
        {
            ...
            {
                if (!_strings.Contains(entry.Name)) _strings.Add(entry.Name);
            ...
            var size = (Version == 4 ? SHP0v4.Size : SHP0v3.Size) + 0x18 + Children.Count * 0x10 + _strings.Count * 4;
            foreach (SHP0EntryNode entry in Children) size += entry.CalculateSize(true) + entry.Children.Count * 4;
            ...
            StringTable stringTable)
        {
            ...
            var stringPtr = header->StringEntries;
            for (var i = 0; i < header->_numEntries; i++)
        ...
        public void Resize(int newFrameCount)
        {
            ...
            {
                var newArr = new KeyframeArray(newFrameCount);
        ...
        public void Append(SHP0Node external)
        {
            ...
            foreach (SHP0VertexSetNode extEntry in w.Children)
            {
                ...
                if ((intEntry = (SHP0VertexSetNode) FindChild(w.Name + "/" + extEntry.Name, false)) == null)
                {
                    ...
                    var newIntEntry = new SHP0VertexSetNode(extEntry.Name);
        ...
        {
            var w = FindChild(baseName, false) as SHP0EntryNode;
            if (w == null) return;

            var t = w.FindChild(morphName, false) as SHP0VertexSetNode;
            if (t == null) return;

            if (t.Keyframes._keyCount > 1)
            {
                var root = t.Keyframes._keyRoot;
                if (root._next != root && root._prev != root && root._prev != root._next)
                {
                    var tan = (root._next._tangent + root._prev._tangent) / 2.0f;
                    var val = (root._next._value + root._prev._value) / 2.0f;
    ...
    public unsafe class SHP0EntryNode : ResourceNode
    {
        ...
        public override bool OnInitialize()
        {
            ...
            for (var i = 0; i < Header->_numIndices; i++) _indices.Add(Header->Indicies[i]);
        ...
        public override void OnPopulate()
        {
            for (var i = 0; i < _indexCount; i++)
            {
                ...
                var n = new SHP0VertexSetNode(name);
        ...
        {
            var header = (SHP0Entry*) dataAddress;
            header->ResourceStringAddress = stringTable[Name] + 4;
        }
        ...
        public override int OnCalculateSize(bool force)
        {
            ...
            foreach (SHP0VertexSetNode p in Children) _dataLen += p.CalculateSize(true);
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            ...
            foreach (SHP0VertexSetNode p in Children)
            {
                ...
                if (p._isFixed)
                {
                    float value = 0;
                    KeyframeEntry kf;
                    if ((kf = p.Keyframes.GetKeyframe(0)) != null) value = kf._value;
        ...
        public SHP0VertexSetNode FindOrCreateEntry(string name)
        {
            ...
            var entry = new SHP0VertexSetNode(name);
            AddChild(entry);
            return entry;
        }
        ...
        {
            var morph = new SHP0VertexSetNode(FindName("NewMorphTarget"));
            AddChild(morph);
        }
    }
    ...
    public unsafe class SHP0VertexSetNode : ResourceNode, IKeyframeSource
    {
        ...
        public override void OnRebuild(VoidPtr address, int length, bool force)
        {
            var header = (SHP0KeyframeEntries*) address;
        ...
        {
            var length = OnCalculateSize(true);
        ...
        {
            var exists = Keyframes.GetKeyframe(index) != null;
            var k = Keyframes.SetFrameValue(index, value);

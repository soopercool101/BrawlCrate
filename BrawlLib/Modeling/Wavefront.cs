...
namespace BrawlLib.Modeling
{
    public static class Wavefront
    {
        ...
        {
            using (var stream = new FileStream(outPath, FileMode.Create, FileAccess.ReadWrite, FileShare.None))
            using (var writer = new StreamWriter(stream))
            {
                writer.WriteLine("#Wavefront OBJ file, generated by {0} v{1}", Application.ProductName,
                ...
                foreach (var o in assets)
                    if (o is MDL0VertexNode)
                        ...
                        foreach (var c in ((MDL0ObjectNode) o)._drawCalls)
        ...
        private static void WriteVertexGroup(StreamWriter writer, Vector3[] verts)
        {
            ...
            foreach (var v in verts) writer.WriteLine("v {0} {1} {2}", v._x, v._y, v._z);
        }
        ...
        private static unsafe void WriteVertexGroup(StreamWriter writer, UnsafeBuffer verts)
        {
            writer.WriteLine();
            writer.WriteLine("#Vertices");
            var pVert = (Vector3*) verts.Address;
            for (var i = 0; i < verts.Length / 12; i++)
            {
        ...
        private static void WriteNormalGroup(StreamWriter writer, MDL0NormalNode norm)
        {
            ...
            foreach (var v in norm.Normals) writer.WriteLine("vn {0} {1} {2}", v._x, v._y, v._z);
        }
        ...
        private static unsafe void WriteNormalGroup(StreamWriter writer, UnsafeBuffer norms)
        {
            writer.WriteLine();
            writer.WriteLine("#Normals");
            var pVert = (Vector3*) norms.Address;
            for (var i = 0; i < norms.Length / 12; i++)
            {
        ...
        private static unsafe void WriteNormalGroup(StreamWriter writer, PrimitiveManager p, bool weight)
        {
            ...
            writer.WriteLine();
            writer.WriteLine("#Normals");
            var pVert = (Vector3*) p._faceData[1].Address;
            for (var i = 0; i < p._faceData[1].Length / 12; i++)
            {
                //writer.WriteLine("vn {0} {1} {2}", pVert->_x, pVert->_y, (pVert++)->_z);
        ...
        private static void WriteUVGroup(StreamWriter writer, MDL0UVNode uv)
        {
            ...
            foreach (var v in uv.Points) writer.WriteLine("vt {0} {1}", v._x, v._y);
        }
        ...
        {
            writer.WriteLine("usemtl {0}", mat.Name);
        }

        private static void WritePolygon(StreamWriter writer, DrawCall c)
        {
            var poly = c._parentObject;

            if (poly._manager._vertices != null)
            {
                var count = poly._manager._vertices.Count;
                ...
                for (var i = 0; i < count; i++) Vertices[i] = poly._manager._vertices[i].WeightedPosition;

                WriteVertexGroup(writer, Vertices);
            }

            if (poly._manager._faceData[1] != null) WriteNormalGroup(writer, poly._manager, true);

            for (var i = 0; i < 1; i++) //Obj only supports 1 uv coord set...
                if (poly._manager._faceData[i + 4] != null)
                    WriteUVGroup(writer, poly._manager._faceData[i + 4]);

            writer.WriteLine();
            writer.WriteLine("g {0}", poly.Name);
            if (c._material != null) WriteMaterial(writer, c._material);
        ...
        {
            var pData = p._triangles._indices;
            uint x = 0;

            var pVert = (ushort*) p._indices.Address;

            var hasUVs = p._faceData[4] != null;
            var hasNorms = p._faceData[1] != null;

            writer.WriteLine();
            writer.WriteLine("#Trilist");

            var count = p._triangles._indices.Length / 3;

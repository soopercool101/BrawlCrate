    ...
    public unsafe partial class Collada : Form
    {
        ...
        public IModel ImportModel(string filePath, ImportType type)
        {
            ...
            switch (type)
            {
                case ImportType.MDL0:
                    ...
                    if (_importOptions._setOrigPath) m._originalPath = filePath;
            ...
            using (var shell = DecoderShell.Import(filePath))
            {
                try
                {
                    ...
                    foreach (var img in shell._images)
                    {
                        var name = img._path != null
                    ...
                    {
                        case ImportType.MDL0:
                            var m = (MDL0Node) model;
                            var shadNode = new MDL0ShaderNode
                            {
                                _ref0 = 0,
                                _ref1 = -1,
                                _ref2 = -1,
                    ...
                    //Extract materials
                    foreach (var mat in shell._materials)
                    {
                        var uwrap = new List<int>();
                        var vwrap = new List<int>();
                        var minfilter = new List<int>();
                        var magfilter = new List<int>();
                        var imgEntries = new List<ImageEntry>();

                        //Find effect
                        if (mat._effect != null)
                            foreach (var eff in shell._effects)
                                if (eff._id == mat._effect) //Attach textures and effects to material
                                    if (eff._shader != null)
                                        foreach (var l in eff._shader._effects)
                                            if (l._type == LightEffectType.diffuse && l._texture != null)
                                            {
                                                var path = l._texture;
                                                foreach (var p in eff._newParams)
                                                    if (p._sid == l._texture)
                                                    {
                                                        path = p._sampler2D._url;
                                                        if (!string.IsNullOrEmpty(p._sampler2D._source))
                                                            foreach (var p2 in eff._newParams)
                        ...
                        {
                            case ImportType.MDL0:
                                var matNode = new MDL0MaterialNode();
                    ...
                    var _objects = new List<ObjectInfo>();
                    ...
                    foreach (var scene in shell._scenes)
                    ...
                    {
                        var node = obj._node;
                        var w = obj._weighted ? "" : "un";
                        var w2 = obj._weighted ? "\nOne or more vertices may not be weighted correctly." : "";
                        var n = node._name ?? node._id;
            ...
            {
                var m = (MDL0Node) model;
                var oldTop = m.FindBoneByIndex(0);
                if (oldTop.HasChildren)
                {
                    var actualTop = (MDL0BoneNode) oldTop.Children[0];
                    ...
                    if (oldTop.Children.Count == 1 && (!oldTop.Name.Equals("TopN") || actualTop.Name.Equals("TopN")))
                    {
                        ...
                        foreach (MDL0ObjectNode obj in m._objList)
                        {
                            ...
                            foreach (var dc in obj._drawCalls)
            ...
            Matrix parentInvMatrix)
        {
                ...
                node._type == NodeType.NONE && node._instances.Count == 0 && (node._name != null || node._id != null))
            {
                ...
                if (inf != null) model.Influences._influences.Add(inf);
            }

            //parentInvMatrix *= node._matrix.Invert();
            foreach (var e in node._children)
                EnumNode(e, parent, scene, model, shell, objects, bindMatrix, parentInvMatrix);

            foreach (var inst in node._instances)
                if (inst._type == InstanceType.Controller)
                    foreach (var skin in shell._skins)
                        if (skin._id == inst._url)
                        {
                            foreach (var g in shell._geometry)
            ...
            DecoderShell shell)
        {
            if (manager != null)
            {
                ...
                {
                    var bone = (MDL0BoneNode) parent;
                    poly.DeferUpdateAssets();
                    poly.MatrixNode = bone;

                    foreach (var c in poly._drawCalls) c.VisibilityBoneNode = bone;
                }
                ...
                else
                {
                    ...
                    foreach (var c in poly._drawCalls) c.VisibilityBoneNode = model._boneList[0] as MDL0BoneNode;
        ...
        private void FinishMDL0(MDL0Node model)
        {
            ...
            {
                var mat = new MDL0MaterialNode {_name = "Default"};
                ...
                {
                    if (obj._drawCalls.Count == 0) obj._drawCalls.Add(new DrawCall(obj));
            ...
            {
                var pixel = _importOptions._dfltClr;
                ...
                if (model._objList != null && model._objList.Count != 0)
                    foreach (MDL0ObjectNode p in model._objList)
                        ...
                        {
                            var pIn = (RGBAPixel*) (p._manager._faceData[2] =
                                new UnsafeBuffer(4 * p._manager._pointCount)).Address;
                            for (var i = 0; i < p._manager._pointCount; i++) *pIn++ = pixel;
                        }
            }
            ...
            if (_importOptions._rmpMats && model._matList != null && model._objList != null)
            {
                ...
                {
                    if (obj3.DrawCalls.Count == 0) continue;

                    var mat = obj3._drawCalls[0].MaterialNode;
                ...
                for (var i = 0; i < model._matList.Count; i++)
            ...
            if (model != null) model.FinishImport();
        }

        private class ObjectInfo
        {
            public readonly Matrix _bindMatrix;
            public readonly GeometryEntry _g;
            public readonly InstanceEntry _inst;
            public readonly NodeEntry _node;
            public readonly ResourceNode _parent;
            public readonly SceneEntry _scene;
            public readonly SkinEntry _skin;
            public readonly bool _weighted;

...
namespace BrawlLib.Modeling.Triangle_Converter
{
    ...
    public class TriStripper
    {
        ...
        private void InitTriHeap()
        {
            ...
            for (uint i = 0; i < m_Triangles.Count; i++) m_TriHeap.Push(m_Triangles[i].Size);
            ...
            while (!m_TriHeap.Empty && m_TriHeap.Top == 0) m_TriHeap.Pop();
        }
        ...
        private void Stripify()
        {
            while (!m_TriHeap.Empty)
            {
                ...
                var HeapTop = m_TriHeap.Position(0);
                ...
                while (m_Candidates.Count != 0)
                {
                    ...
                    var TriStrip = FindBestStrip();

                    if (TriStrip.Size >= m_MinStripSize) BuildStrip(TriStrip);
                }

                if (!m_TriHeap.Removed(HeapTop)) m_TriHeap.Erase(HeapTop);
        ...
        private void AddRemainingTriangles()
        {
            ...
            var p = new Primitive(PrimType.TriangleList);
            ...
            if (p.Indices.Count > 0) m_PrimitivesVector.Add(p);
        }

        private void ResetStripIDs()
        {
            foreach (Triangle r in m_Triangles) r.ResetStripID();
        }
        ...
        private Strip FindBestStrip()
        {
            ...
            var CacheBackup = m_Cache;

            var policy = new Policy(m_MinStripSize, Cache);

            while (m_Candidates.Count != 0)
            {
                var Candidate = m_Candidates[m_Candidates.Count - 1];
                m_Candidates.RemoveAt(m_Candidates.Count - 1);

                //Discard useless triangles from the candidate list
                if (m_Triangles[Candidate].Marked || m_TriHeap[Candidate] == 0) continue;

                //Try to extend the triangle in the 3 possible forward directions
                for (uint i = 0; i < 3; i++)
                {
                    var Strip = ExtendToStrip(Candidate, (TriOrder) i);
                ...
                if (m_BackwardSearch)
                {
                    for (uint i = 0; i < 3; i++)
                    {
                        var Strip = BackExtendToStrip(Candidate, (TriOrder) i, false);
                        if (Strip != null) policy.Challenge(Strip, m_TriHeap[Strip.Start], m_Cache.HitCount);
                    ...
                    for (uint i = 0; i < 3; i++)
                    {
                        var Strip = BackExtendToStrip(Candidate, (TriOrder) i, true);
                        if (Strip != null) policy.Challenge(Strip, m_TriHeap[Strip.Start], m_Cache.HitCount);
        ...
        private Strip ExtendToStrip(uint Start, TriOrder Order)
        {
            ...
            //Begin a new strip
            var tri = m_Triangles[Start].m_Elem;
            ...
            for (var i = Start; i < m_Triangles.Count && (!Cache || Size + 2 < CacheSize); Size++)
            {
        ...
        private Strip BackExtendToStrip(uint Start, TriOrder Order, bool ClockWise)
        {
            ...
            //Loop while we can further extend the strip
            for (var i = Start; !Cache || Size + 2 < CacheSize; Size++)
            {
                Node = m_Triangles[i];
        ...
        {
            if (!_checkNodes) return true;

            var node = m_Nodes[m_ImpTable[index]];
            if (!m_CurrentNodes.Contains(node))
            {
                if (m_CurrentNodes.Count == PrimitiveGroup._nodeCountMax) return false;
        ...
        {
            var Edge = LastEdge(Node.m_Elem, Order);
            for (var i = Node.m_Begin; i < Node.m_End; i++)
            {
                var Link = Node.Arcs[(int) i];

                //Get the reference to the possible next triangle
                var Tri = Link.Terminal.m_Elem;
        ...
        private GraphArray<Triangle>.Arc BackLinkToNeighbour(GraphArray<Triangle>.Node Node, bool ClockWise,
            ref TriOrder Order)
        {
            var Edge = FirstEdge(Node.m_Elem, Order);
            for (var i = Node.m_Begin; i < Node.m_End; i++)
            {
                var Link = Node.Arcs[(int) i];
        ...
        {
            var Start = Strip.Start;

            var ClockWise = false;
            var Order = Strip.Order;

            //Create a new strip
            var p = new Primitive(PrimType.TriangleStrip);
        ...
        {
            if (Cache) m_Cache.Push(i, !NotSimulation);

            if (NotSimulation) m_PrimitivesVector[m_PrimitivesVector.Count - 1].Indices.Add(i);
        }
        ...
        {
            foreach (GraphArray<Triangle>.Node t in G) t.Marked = false;
        }
        ...
        private void MakeConnectivityGraph(GraphArray<Triangle> Triangles, uint[] Indices)
        {
            ...
            var EdgeMap = new List<TriEdge>();
            for (uint i = 0; i < Triangles.Count; i++)
            {
                var Tri = Triangles[i].m_Elem;
        ...
        private void LinkNeighbours(GraphArray<Triangle> Triangles, List<TriEdge> EdgeMap, TriEdge Edge)
        {
            ...
            for (var i = BinarySearch(EdgeMap, Edge, EdgeComp);

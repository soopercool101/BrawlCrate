...
namespace BrawlLib.Modeling.Triangle_Converter
{
    internal class CacheSimulator
    {
        ...
        public void Resize(uint Size)
        {
            ...
            m_Cache.Clear();
            for (var x = 0; x < Size; x++) m_Cache.PushFront(uint.MaxValue);
        }
        ...
        public void Reset()
        {
            ...
            for (var x = 0; x < m_Cache.Count; x++) m_Cache.PushFront(uint.MaxValue);
        ...
        {
            var Overlap = Math.Min(PossibleOverlap, Size);

            for (uint i = 0; i < Overlap; ++i) Push(Backward.m_Cache[i], true);

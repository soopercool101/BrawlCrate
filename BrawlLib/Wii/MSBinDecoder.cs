...
namespace BrawlLib.Wii
{
    public static unsafe class MSBinDecoder
    {
        internal static string DecodeString(byte* sPtr, int len)
        {
            ...
            var input_count = (int) dPtr - (int) buffer;
            var output_count = Encoding.UTF8.GetDecoder().GetCharCount(buffer, input_count, true);
            var buffer2 = new char[output_count];
        ...
        public static int EncodeString(string s, byte* dPtr)
        {
            ...
            var utf8_to_char_array = new char[utf8.Length];
            for (var i = 0; i < utf8.Length; i++) utf8_to_char_array[i] = (char) utf8[i];
                ...
                while (sPtr < ceil)
                {
                    ...
                    if (c == '<')
                    {
                        ...
                        else
                        {
                            ...
                            if (StrContains(buffer, bLen, "end")) *dPtr++ = 0x13;
                        }
                    }
                    else if (c == '[')
                    {
                ...
                while (sPtr < ceil)
                {
                    ...
                    if (c == '<')
                    {
                        ...
                        else
                        {
                            ...
                            if (hasColor = StrContains(buffer, bLen, "color=")) len += 5;

                            if (hasSize = StrContains(buffer, bLen, "size=")) len += 3;

                            if (hasSize || hasColor) len += 2;

                            if (StrContains(buffer, bLen, "end")) len++;
                        }
                    }
                    else if (c == '[')
                    {
                        ...
                        else
                        {
                            ...
                            if (StrContains(buffer, bLen, "border=")) len += 5;
                        }
                    }
                    else if (c == '\\')
                    {
        ...
        private static void WriteString(ref byte* dPtr, string value)
        {
            ...
            {
                for (var i = 0; i < len;) *dPtr++ = (byte) sPtr[i++];
            }
        }
        ...
        {
            byte b;
            for (var ceil = ptr + len; ptr < ceil; ptr++)
            {
                b = *ptr;
                if (b > 0x40 && b < 0x5B) *ptr = (byte) (b + 0x20);
            }
        }

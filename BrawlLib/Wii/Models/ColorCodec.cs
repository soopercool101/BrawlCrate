...
namespace BrawlLib.Wii.Models
{
    ...
    public unsafe class ColorCodec : IDisposable
    {
        ...
        private void Evaluate()
        {
            ...
            var imp = _remapData._impTable;
            var impLen = imp.Length;
        ...
        public void Write(byte* pOut)
        {
            try
            {
                //Write colors
                byte* sPtr;
                foreach (var i in _remapData._impTable)
                {
                    sPtr = (byte*) (_pData + i);
                ...
                //Zero-fill
                for (var x = _dataLen; (x & 0x1F) != 0; x++) *pOut++ = 0;
            }
            finally
            {
                Dispose();
        ...
        {
            var sPtr = (byte*) (_pData + _remapData._impTable[index]);
            enc(ref sPtr, ref pOut);
        }
        ...
        public static UnsafeBuffer Decode(MDL0ColorData* header)
        {
            ...
            var buffer = new UnsafeBuffer(count * 4);
            var pIn = (byte*) header + header->_dataOffset;
            var pOut = (byte*) buffer.Address;
            ...
            while (count-- > 0) dec(ref pIn, ref pOut);
        ...
        public static ARGBPixel[] ExtractColors(MDL0ColorData* colors)
        {
            ...
            fixed (ARGBPixel* p = c)
            {
                ...
                switch (colors->Type)
                {
                    case WiiColorComponentType.RGB565:
                    {
                        var sPtr = (wRGB565Pixel*) colors->Data;
                        for (var i = 0; i < count; i++) *dPtr++ = (ARGBPixel) (*sPtr++);
                    ...
                    case WiiColorComponentType.RGB8:
                    {
                        var sPtr = (wRGBPixel*) colors->Data;
                        for (var i = 0; i < count; i++) *dPtr++ = (ARGBPixel) (*sPtr++);
                    ...
                    case WiiColorComponentType.RGBX8:
                    {
                        var sPtr = (wRGBXPixel*) colors->Data;
                        for (var i = 0; i < count; i++) *dPtr++ = (ARGBPixel) (*sPtr++);
                    ...
                    case WiiColorComponentType.RGBA4:
                    {
                        var sPtr = (wRGBA4Pixel*) colors->Data;
                        for (var i = 0; i < count; i++) *dPtr++ = (ARGBPixel) (*sPtr++);
                    ...
                    case WiiColorComponentType.RGBA6:
                    {
                        var sPtr = (wRGBA6Pixel*) colors->Data;
                        for (var i = 0; i < count; i++) *dPtr++ = (ARGBPixel) (*sPtr++);
                    ...
                    case WiiColorComponentType.RGBA8:
                    {
                        var sPtr = (wRGBAPixel*) colors->Data;
                        for (var i = 0; i < count; i++) *dPtr++ = (ARGBPixel) (*sPtr++);

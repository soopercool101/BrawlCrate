            ...
            try
            {
                ...
                for (TextReader reader = new StreamReader(input); reader.Peek() != -1;)
                {
                    ...
                    if (line.CompareTo("Animation Keyframe Data Frames") == 0)
                    {
                        ...
                        var justNumbers = new string(line.Where(char.IsDigit).ToArray());
                        var keyFrameCount = int.Parse(justNumbers);
                        ...
                        for (line = reader.ReadLine();
                            ...
                            {
                                var node = new CHR0EntryNode();
                                chr0.AddChild(node);
                                node.Name = reader.ReadLine();
                                node.SetSize(keyFrameCount, chr0.Loop);

                                var translation = new Coords();
                                var rotation = new Coords();

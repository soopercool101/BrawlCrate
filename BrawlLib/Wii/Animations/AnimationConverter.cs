...
namespace BrawlLib.Wii.Animations
{
    internal static unsafe class AnimationConverter
    {
            ...
            params float[] defaults)
        {
            ...
            var numFrames = node == null ? 1 : node.FrameCount + (node.Loop ? 1 : 0);
        ...
        public static KeyframeArray DecodeSHP0Keyframes(SHP0KeyframeEntries* entry, SHP0Node node)
        {
            ...
            var kf = new KeyframeArray(numFrames) {Loop = node.Loop};

            if (entry == null) return kf;

            int fCount = entry->_numEntries;
            var vec = entry->Entries;
            for (var i = 0; i < fCount; i++, vec++) kf.SetFrameValue((int) vec->_y, vec->_z, true)._tangent = vec->_x;
        ...
        {
            var kf = new KeyframeCollection(5, numFrames, 1, 1);

            if (entry == null) return kf;

            var sPtr = (bfloat*) entry->Data;
            var code = entry->Code;
        ...
        {
            var kf = new KeyframeCollection(9, numFrames, 1, 1, 1);
            ...
            params int[] arrays)
        {
            ...
            switch (format)
            {
                case AnimDataFormat.I4:
                {
                    ...
                    {
                        var entry = header->Data;
                        for (var i = 0; i < fCount; i++, entry++)
                ...
                {
                    var header = (L1Header*) dataAddr;
                    vStep = header->_step;
                    vBase = header->_base;

                    foreach (var x in arrays)
                    {
                        var sPtr = header->Data;
                        for (var i = 0; i < kf.FrameLimit; i++)
                ...
                case AnimDataFormat.L4:
                {
                    foreach (var x in arrays)
                    {
                        var sPtr = (bfloat*) dataAddr;
                        for (var i = 0; i < kf.FrameLimit; i++) kf.SetFrameValue(x, i, *sPtr++, true).GenerateTangent();
                    }
        ...
        public static int CalculateCHR0Size(KeyframeCollection kf, out int entrySize, out AnimationCode code)
        {
            ...
            for (var i = 0; i < 3; i++) dataSize += EvaluateCHR0Group(ref code, kf, i, ref entrySize);
        ...
        public static int CalculateSRT0Size(KeyframeCollection kf, out int entrySize, out SRT0Code code)
        {
            ...
            for (var i = 0; i < 3; i++) dataSize += EvaluateSRT0Group(ref code, kf, i, ref entrySize);
            ...
            ref int entrySize)
        {
            ...
            var exist = false;
            var isotropic = group == 0;
            var format = AnimDataFormat.None;

            var roots = new KeyframeEntry[3];

            var arr = new KeyframeEntry[3][];
            var count = new int[3];
            var isExist = new bool[3];
            var isFixed = new bool[3];
            var isScalable = new bool[3];
            var floor = new float[3];
            var ceil = new float[3];
            ...
            if (exist = isExist[0] || isExist[1] || isExist[2])
            {
                ...
                {
                    isScalable[i] = true;
                    if (isFixed[i] || scaleSpan == -1) continue;
                    ...
                    SpanBegin:
                    var span = scaleSpan;
                    var spanEval = scaleSpan - 32;

                    var tanScale = scaleSpan == 4095 ? 32.0f : 256.0f;
                    if (scaleSpan != 255)
                    ...
                    if (span > spanEval && range == 0.0f) continue;
                ...
                {
                    var scale = isotropic ? isScalable[0] : isScalable[0] && isScalable[1] && isScalable[2];
                    var frameSpan = (float) numFrames / maxEntries;
            ...
            if (group == 0) code.IgnoreScale = !exist;

            code.SetExists(group, exist);
            code.SetIsIsotropic(group, isotropic);
            for (var i = 0; i < 3; i++) code.SetIsFixed(index + i, isFixed[i]);
        ...
        private static int EvaluateSRT0Group(ref SRT0Code code, KeyframeCollection kf, int group, ref int entrySize)
        {
            ...
            var index = group == 0 ? 0 : group == 1 ? 2 : 3;
            var numFrames = kf.FrameLimit;
            var dataLen = 0;
            var roots = new KeyframeEntry[2];
            var exist = false;
            var isotropic = group == 0;
            var count = new int[2];
            var isExist = new bool[2];
            var isFixed = new bool[2];

            for (var i = 0; i < (group == 1 ? 1 : 2); i++)
            {
            ...
            if (group == 0 && !isotropic) code.ScaleIsotropic = false;
            ...
            for (var i = 0; i < (group == 1 ? 1 : 2); i++)
                if (exist)
                {
                    ...
                    if (!(group == 0 && i == 1 && code.ScaleIsotropic)) entrySize += 4;
                    ...
                    if (!isFixed[i])
                    {
                        ...
                        if (!(group == 0 && i == 1 && code.ScaleIsotropic)) dataLen += 8 + count[i] * 12;
                    }
                }
        ...
        public static void EncodeSRT0Keyframes(KeyframeCollection kf, VoidPtr entryAddress, VoidPtr dataAddress,
            SRT0Code code)
        {
            var header = (SRT0TextureEntry*) entryAddress;
            header->Code = code;

            var pOffset = (bint*) entryAddress + 1;
            ...
            for (var type = 0; type < 3; type++)
            {
                ...
                for (var axis = 0; axis < (type == 1 ? 1 : 2); axis++, index++)
                    if (has)
                    {
        ...
        private static int EncodeEntry(int index, AnimDataFormat format, KeyframeCollection kf, VoidPtr addr)
        {
            ...
            if (format == AnimDataFormat.L1)
            {
                ...
                var header = (L1Header*) addr;
                *header = new L1Header(step, min);

                var dPtr = header->Data;
                for (i = 0; i < numFrames; i++) *dPtr++ = (byte) ((kf[i, index] - min) / step + 0.5f);
            ...
            bool evalAll)
        {
            ...
            if (maxIterations <= 0) maxIterations = maxSpan - 2;

...
namespace BrawlLib.Wii.Compression
{
    ...
    public static unsafe class Compressor
    {
        ...
        {
            var compTag = *(BinTag*) addr;
            if (compTag == YAZ0.Tag) return CompressionType.RunLengthYAZ0;

            if (compTag == YAY0.Tag) return CompressionType.RunLengthYAY0;

            var cmpr = (CompressionHeader*) addr;

            if (cmpr->ExpandedSize < length) return CompressionType.None;

            if (!cmpr->HasLegitCompression()) return CompressionType.None;

            //Check to make sure we're not reading a tag
            if (IsTag((byte*) addr)) return CompressionType.None;
        ...
        {
            var chars = _characters.ToCharArray();
        ...
        {
            var buffer = stackalloc byte[CompressBufferLen];
            ...
            if (NodeFactory.GetRaw(buffer, CompressBufferLen) != null) return true;
        ...
        public static FileMap TryExpand(ref DataSource source, bool doTest = true)
        {
            ...
            var algorithm = GetAlgorithm(source);
            if (Supports(source.Compression = algorithm))
                try
                {
                    if (doTest && !Test(algorithm, source.Address)) return null;
                    ...
                    {
                        var hdr = (CompressionHeader*) source.Address;
        ...
        public static void Compact(CompressionType type, VoidPtr srcAddr, int srcLen, Stream outStream, ResourceNode r)
        {
            switch (type)
            {
                case CompressionType.LZ77:

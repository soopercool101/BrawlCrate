...
namespace System.Windows.Forms
{
    public partial class ModelEditControl : ModelEditorBase
    {
        ...
        {
            var curViewport = modelPanel.CurrentViewport;
            var newViewport = ModelPanelViewport.DefaultPerspective;
            newViewport.BackgroundColor = curViewport.BackgroundColor;
            ModelPanel.AddViewport(newViewport);

            var xMin = curViewport.Percentages._x;
            var yMin = curViewport.Percentages._y;
            var xMax = curViewport.Percentages._z;
            var yMax = curViewport.Percentages._w;
            var averageX = (xMin + xMax) / 2.0f;
        ...
        private void LiveTextureFolderPath_Click(object sender, EventArgs e)
        {
            ...
            if (string.IsNullOrEmpty(LiveTextureFolderPath.Text))
                LiveTextureFolderPath.Text = Path.GetDirectoryName(Assembly.GetEntryAssembly().Location);
        ...
        {
            if (TargetModel == null || !(TargetModel is MDL0Node)) return;

            var node = TargetAnimation;
            if (node is CHR0Node) (node as CHR0Node).Port((MDL0Node) TargetModel);
        ...
        private void mergeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            var node = TargetAnimation;
            if (node is CHR0Node) (node as CHR0Node).MergeWith();
        ...
        private void appendToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else if (node is VIS0Node) (node as VIS0Node).Append();
        ...
        private void resizeToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            else if (node is VIS0Node) (node as VIS0Node).Resize();
        ...
        {
            var n = TargetAnimation;
            if (n is CHR0Node) ((CHR0Node) n).AverageKeys();

            if (n is SRT0Node) ((SRT0Node) n).AverageKeys();

            if (n is SHP0Node) ((SHP0Node) n).AverageKeys();
        }

        private void averageboneStartendTangentsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            var n = TargetAnimation;
            if (n is CHR0Node && SelectedBone != null) ((CHR0Node) n).AverageKeys(SelectedBone.Name);
        ...
        private void ScreenCapBgLocText_Click(object sender, EventArgs e)
        {
            using (var d = new FolderBrowserDialog())
            {
                ...
                if (d.ShowDialog(this) == DialogResult.OK) ScreenCapBgLocText.Text = d.SelectedPath;
            }
        ...
        private void imageFormatToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            using (var d = new ExportAllFormatDialog())
            {
        ...
        private void btnExportToAnimatedGIF_Click(object sender, EventArgs e)
        {
            ...
            if (InterpolationEditor != null) InterpolationEditor.Enabled = false;
        ...
        private void detachViewerToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (_viewerForm == null)
            {
                ...
                foreach (ModelPanelViewport v in modelPanel) _viewerForm.modelPanel1.AddViewport(v);
        ...
        {
            foreach (var node in _targetModels)
                if (node != TargetModel)

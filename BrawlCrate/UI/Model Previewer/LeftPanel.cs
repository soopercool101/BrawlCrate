...
namespace System.Windows.Forms
{
    public class LeftPanel : UserControl
    {
        ...
        public NW4RAnimType TargetAnimType
        {
            ...
            set
            {
                if (fileType.SelectedIndex != (int) value) fileType.SelectedIndex = (int) value;
            }
        }
        ...
        {
            var ib = inBRRES || TargetModel != null && TargetModel is MDL0Node &&
                     node == ((MDL0Node) TargetModel).BRESNode;

            if (!_mainWindow.chkBRRESAnims.Checked && ib) return false;
            ...
            switch (node.ResourceFileType)
            {
                ...
                case ResourceType.CHR0:
                    found = true;
                    if (type == NW4RAnimType.CHR) goto Add;
                    break;
                case ResourceType.SRT0:
                    found = true;
                    if (type == NW4RAnimType.SRT) goto Add;
                    break;
                case ResourceType.SHP0:
                    found = true;
                    if (type == NW4RAnimType.SHP) goto Add;
                    break;
                case ResourceType.PAT0:
                    found = true;
                    if (type == NW4RAnimType.PAT) goto Add;
                    break;
                case ResourceType.VIS0:
                    found = true;
                    if (type == NW4RAnimType.VIS) goto Add;
                    break;
                case ResourceType.SCN0:
                    found = true;
                    if (type == NW4RAnimType.SCN) goto Add;
                    break;
                case ResourceType.CLR0:
                    found = true;
                    if (type == NW4RAnimType.CLR) goto Add;
                    break;
            }
            ...
            {
                var u = externalGroup ?? (ib ? _AnimGroupBRRES : _AnimGroupNotBRRES);
        ...
        public void UpdateAnimations(NW4RAnimType type)
        {
            ...
            var name = listAnims.SelectedItems != null && listAnims.SelectedItems.Count > 0
                ? listAnims.SelectedItems[0].Tag.ToString()
                : null;
            var frame = CurrentFrame;

            var text = txtSearchAnim.Text;
            var addAll = string.IsNullOrEmpty(text) || txtSearchAnim.ForeColor == Color.Gray;
            ...
            if (TargetModel != null && TargetModel is MDL0Node)
            {
                ...
                var node = _mainWindow.chkNonBRRESAnims.Checked
            ...
            {
                var root = TargetModel == null ? null : ((ResourceNode) TargetModel).RootNode;
                foreach (var r in _mainWindow._openedFiles)
                    if (r != root && r != null)
                    {
                        var g = new ListViewGroup(r.Name);
            ...
            for (var i = 0; i < listAnims.Items.Count; i++)
        ...
        public void LoadAnimations(ResourceNode node)
        {
            if (_mainWindow.chkExternalAnims.Checked && node != null)
            {
                ...
                {
                    var g = new ListViewGroup(node.Name);
                    listAnims.Groups.Add(g);
                    _AnimGroupsExternal.Add(g);
        ...
        public void UpdateTextures()
        {
            ...
            var Name = lstTextures.SelectedItems != null && lstTextures.SelectedItems.Count > 0
            ...
            //Reselect the animation
            //lstTextures.Focus();
            for (var i = 0; i < lstTextures.Items.Count; i++)
        ...
        public void Reset()
        {
            ...
            {
                foreach (var obj in TargetCollision._objects) lstObjects.Items.Add(obj, obj._render);
            }
            else if (TargetModel != null)
            {
                UpdateAnimations(TargetAnimType);
        ...
        private void spltAnimObj_Dragged(object sender, SplitterEventArgs e)
        {
            ...
            var texY = pnlTextures.Location.Y;
            var objY = pnlObjects.Location.Y;

            var height = -1;
            if (objY + btnObjects.Height + e.Y >= texY - 6)
            {
                var difference = objY + btnObjects.Height + e.Y - (texY - 6);
            ...
            if (height != pnlTextures.Height) pnlAnims.Height += e.Y;
        }
        ...
        private void spltObjTex_Dragged(object sender, SplitterEventArgs e)
        {
            ...
            if (texY - 6 + e.Y <= objY + btnObjects.Height)
            {
                var difference = objY + btnObjects.Height - (texY - 6 + e.Y);
                if (objY + btnObjects.Height - e.Y >= texY - 6)
            ...
            if (height != pnlAnims.Height) pnlTextures.Height -= e.Y;
        }
        ...
        private void lstPolygons_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            ...
            var poly = lstObjects.Items[e.Index] as MDL0ObjectNode;

            if (poly != null)
            {
                var matches = poly.IsRendering == (e.NewValue == CheckState.Checked);

                if (!matches) poly.IsRendering = e.NewValue == CheckState.Checked;
                ...
                if (!matches && chkSyncVis.Checked) ChecksChanged(poly);
            }

            var obj = lstObjects.Items[e.Index] as CollisionObject;
            if (obj != null) obj._render = e.NewValue == CheckState.Checked;
        ...
        public void ChecksChanged(MDL0ObjectNode poly)
        {
            ...
            foreach (var d in poly._drawCalls)
                if (_mainWindow.VIS0Indices.ContainsKey(d.VisibilityBone))
                {
                    var objects = _mainWindow.VIS0Indices[d.VisibilityBone];
                    foreach (var i in objects)
                    {
                        if (i.Key < 0 || i.Key >= lstObjects.Items.Count || i.Value == null) continue;

                        var o = (MDL0ObjectNode) lstObjects.Items[i.Key];
                        foreach (var call in i.Value)
                        {
                            var other = o._drawCalls[call];
                            if (other._render != d._render && other != d) other._render = d._render;
                        }
            ...
            if (!_mainWindow.VIS0Updating)
            {
                ...
                if (chkSyncVis.Checked)
                    for (var i = 0; i < poly._drawCalls.Count; i++)
        ...
        {
            if (_selectedTexture != null) _selectedTexture.Selected = false;
            ...
            if ((_selectedTexture = lstTextures.SelectedItem as MDL0TextureNode) != null)
            {
                ...
                if (_mainWindow.SyncTexturesToObjectList) _selectedTexture.ObjOnly = true;
        ...
        {
            if (lstObjects.Items.Count == 0) return;

            //_updating = true;

            lstObjects.BeginUpdate();
            for (var i = 0; i < lstObjects.Items.Count; i++) lstObjects.SetItemCheckState(i, chkAllObj.CheckState);
        ...
        {
            var tref = lstTextures.Items[e.Index] as MDL0TextureNode;
        ...
        {
            var index = lstTextures.SelectedIndex;
            if (_selectedTexture != null && _selectedTexture.Source is TEX0Node)
            {
                var node = _selectedTexture.Source as TEX0Node;
        ...
        {
            var index = lstTextures.IndexFromPoint(e.X, e.Y);
            if (lstTextures.SelectedIndex != index) lstTextures.SelectedIndex = index;
        ...
        {
            if (e.KeyCode == Keys.Escape) lstTextures.SelectedItem = null;
        }

        private void lstPolygons_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape) lstObjects.SelectedItem = null;
        }

        private void listAnims_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Escape) listAnims.SelectedItems.Clear();
        }
        ...
        private void exportTextureToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (_selectedTexture != null && _selectedTexture.Source is TEX0Node)
            {
                ...
                using (var dlgSave = new SaveFileDialog())
                {
                    ...
                    if (dlgSave.ShowDialog(this) == DialogResult.OK) node.Export(dlgSave.FileName);
                }
            }
        }
        ...
        private void listAnims_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (listAnims.SelectedItems.Count > 0)
            {
                ...
                var n = listAnims.SelectedItems[0].Tag as NW4RAnimationNode;
                if (n != null) chkLoop.Checked = n.Loop;
        ...
        {
            if (_srt0Selection == null && _pat0Selection == null) return;

            var g = e.Graphics;
            for (var i = 0; i < lstObjects.Items.Count; i++)
            {
                var poly = lstObjects.Items[i] as MDL0ObjectNode;
                if (poly != null)
                    foreach (var c in poly._drawCalls)
                        if (c.MaterialNode != null)
                        {
                            if (_srt0Selection != null)
                            {
                                ...
                                {
                                    var r = lstObjects.GetItemRectangle(i);
        ...
        private void overTexPnl_Paint(object sender, PaintEventArgs e)
        {
            ...
            if (_selectedObject != null && _selectedObject is MDL0ObjectNode)
                for (var i = 0; i < lstTextures.Items.Count; i++)
                {
                    var obj = _selectedObject as MDL0ObjectNode;
                    var tex = lstTextures.Items[i] as MDL0TextureNode;
                    foreach (var c in obj._drawCalls)
                        if (c.MaterialNode != null)
                            if ((rn = c.MaterialNode.FindChild(tex.Name, true)) != null)
                            {
                                if (_srt0Selection != null)
                                {
                                    ...
                                    {
                                        var r = lstTextures.GetItemRectangle(i);
                                        g.DrawRectangle(Pens.Black, r);
                                    }
                                }
                                else if (_pat0Selection != null)
        ...
        private void chkLoop_CheckedChanged(object sender, EventArgs e)
        {
            ...
            var n = _mainWindow.TargetAnimation;
            if (n != null)
            {
        ...
        {
            var r = TargetModel as ResourceNode;
            if (r == null || r.Parent == null || r.Parent.Parent == null || !(r.Parent.Parent is BRRESNode)) return;
        ...
        {
            var t = ModelEditorBase.AnimTypeList[(int) TargetAnimType];
        ...
        {
            var r = TargetModel as ResourceNode;
            var targetBRRES = !(r == null || r.Parent == null || r.Parent.Parent == null ||
                                !(r.Parent.Parent is BRRESNode));

            if (!targetBRRES) e.Cancel = true;
        ...
        private void chkMtxMaya_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                var node = (SRT0Node) _mainWindow.TargetAnimation;
        ...
        private void lstDrawCalls_ItemCheck(object sender, ItemCheckEventArgs e)
        {
            ...
            var c = lstDrawCalls.SelectedItem as DrawCall;
            if (c != null)
            {
                c._render = !c._render;
                UpdateObjectCheckState(lstObjects.SelectedIndex);
                if (chkSyncVis.Checked) ChecksChanged(c._parentObject);
        ...
        {
            if (i < 0 || i > lstObjects.Items.Count) return;

            var o = lstObjects.Items[i] as MDL0ObjectNode;
            if (o == null) return;

            var s = CheckState.Indeterminate;
            bool someRendering = false, someNotRendering = false;
            foreach (var x in o._drawCalls)
            {
                if (x._render)
                    someRendering = true;
                else
                ...
                if (someNotRendering && someRendering) break;
            }

            if (someRendering && !someNotRendering)
                s = CheckState.Checked;
            else if (!someRendering && someNotRendering) s = CheckState.Unchecked;
        ...
        private void InitializeComponent()
        {
            ...
            var listViewGroup2 = new ListViewGroup("Animations", HorizontalAlignment.Left);
            ...
            pnlObjects.Size = new Drawing.Size(172, 150);
            ...
            chkAllObj.Size = new Drawing.Size(170, 20);
            ...
            chkSyncVis.Size = new Drawing.Size(170, 20);
            ...
            pnlAnims.Size = new Drawing.Size(172, 178);
            pnlAnims.TabIndex = 2;
            // 
            // listAnims
            // 
            ...
            // ctxAnimList
            // 
            ctxAnimList.ImageScalingSize = new Drawing.Size(20, 20);
            ...
            chkContains.Size = new Drawing.Size(88, 21);
            ...
            // ctxTextures
            // 
            ctxTextures.ImageScalingSize = new Drawing.Size(20, 20);
            ...
            sourceToolStripMenuItem.Size = new Drawing.Size(146, 26);
            ...
            sizeToolStripMenuItem.Size = new Drawing.Size(146, 26);
            sizeToolStripMenuItem.Text = "Size";
            // 
            // toolStripMenuItem1
            // 
            toolStripMenuItem1.Name = "toolStripMenuItem1";
            toolStripMenuItem1.Size = new Drawing.Size(143, 6);
            // 
            // viewToolStripMenuItem
            // 
            ...
            pnlTextures.Size = new Drawing.Size(172, 164);
            ...
            chkAllTextures.Size = new Drawing.Size(170, 20);
            ...
            // ctxAnim
            // 
            ctxAnim.ImageScalingSize = new Drawing.Size(20, 20);
            ...
            toolStripMenuItem2.Size = new Drawing.Size(234, 26);
            toolStripMenuItem2.Text = "Source";
            // 
            // toolStripSeparator1
            // 
            toolStripSeparator1.Name = "toolStripSeparator1";
            toolStripSeparator1.Size = new Drawing.Size(231, 6);
            // 
            // chkLoop
            // 
            ...
            matrixModeToolStripMenuItem.Size = new Drawing.Size(234, 26);
        ...
        {
            BRESEntryNode node;
            if ((node = _mainWindow.TargetAnimation) == null) return;
            ...
            if (dlgSave.ShowDialog() == DialogResult.OK) node.Export(dlgSave.FileName);
        }
        ...
        private void replaceToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (dlgOpen.ShowDialog() == DialogResult.OK) node.Replace(dlgOpen.FileName);
        }

        private void portToolStripMenuItem_Click(object sender, EventArgs e)
        {
            if (TargetAnimType != 0 || SelectedCHR0 == null || !(TargetModel is MDL0Node)) return;

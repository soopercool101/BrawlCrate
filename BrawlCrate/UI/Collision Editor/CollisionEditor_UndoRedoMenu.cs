...
namespace System.Windows.Forms
{
    public class CollisionEditor_UndoRedoMenu : Form
    {
        ...
        {
            UndoRedoListBox = new UndoRedoUIListBox();
            ActionLabel = new Label();
            CollisionStateInfoButton = new Button();
            ClearHistoryButton = new Button();
            UndoRedoSetMaxLimitButton = new Button();

            SuspendLayout();

            // UndoRedoListBox
            UndoRedoListBox.Anchor = AnchorStyles.Top | AnchorStyles.Bottom | AnchorStyles.Left | AnchorStyles.Right;
            UndoRedoListBox.FormattingEnabled = true;
            UndoRedoListBox.IntegralHeight = false;
            UndoRedoListBox.Location = new Drawing.Point(12, 12);
            UndoRedoListBox.Size = new Drawing.Size(246, 263);
            UndoRedoListBox.Name = "UndoRedoListBox";
            UndoRedoListBox.TabIndex = 0;
            UndoRedoListBox.SelectedIndexChanged += UndoRedoListBox_SelectedIndexChanged;
            UndoRedoListBox.MouseDoubleClick += UndoRedoListBox_MouseDoubleClick;
            UndoRedoListBox.KeyPress += UndoRedoListBox_KeyPress;

            // ActionLabel
            ActionLabel.Anchor = AnchorStyles.Bottom | AnchorStyles.Left | AnchorStyles.Right;
            ActionLabel.Location = new Drawing.Point(12, 278);
            ActionLabel.TextAlign = ContentAlignment.MiddleCenter;
            ActionLabel.BorderStyle = BorderStyle.FixedSingle;
            ActionLabel.Size = new Drawing.Size(246, 21);
            ActionLabel.Text = "Undo/Redo ?? Steps";
            ActionLabel.Name = "ActionLabel";
            ActionLabel.AutoSize = false;

            // CollisionStateInfoButton
            CollisionStateInfoButton.Enabled = false;
            CollisionStateInfoButton.Location = new Drawing.Point(11, 302);
            CollisionStateInfoButton.Size = new Drawing.Size(248, 24);
            CollisionStateInfoButton.TabIndex = 1;
            CollisionStateInfoButton.Text = "Selected State Info";
            CollisionStateInfoButton.Name = "CollisionStateInfoButton";
            CollisionStateInfoButton.Click += CollisionStateInfoButton_Click;

            // ClearHistoryButton
            ClearHistoryButton.Anchor = AnchorStyles.Bottom | AnchorStyles.Right;
            ClearHistoryButton.Enabled = false;
            ClearHistoryButton.Location = new Drawing.Point(137, 328);
            ClearHistoryButton.Size = new Drawing.Size(122, 24);
            ClearHistoryButton.TabIndex = 2;
            ...
            UndoRedoSetMaxLimitButton.Anchor = AnchorStyles.Bottom | AnchorStyles.Left;
            UndoRedoSetMaxLimitButton.Enabled = false;
            UndoRedoSetMaxLimitButton.Location = new Drawing.Point(11, 328);
            UndoRedoSetMaxLimitButton.Size = new Drawing.Size(122, 24);
            UndoRedoSetMaxLimitButton.TabIndex = 2;
            UndoRedoSetMaxLimitButton.Text = "Set Max Undo Limit";
            UndoRedoSetMaxLimitButton.Name = "UndoRedoSetMaxLimitButton";
            UndoRedoSetMaxLimitButton.Click += UndoRedoSetMaxLimitButton_Click;

            // CollisionEditor_UndoRedoMenu
            ClientSize = new Drawing.Size(270, 362);
            FormBorderStyle = FormBorderStyle.FixedToolWindow;
            Icon = BrawlLib.Properties.Resources.Icon;
            Name = "CollisionEditor_UndoRedoMenu";
        ...
        private void ClearHistoryButton_Click(object sender, EventArgs e)
        {
            if (MessageBox.Show("Would you like to clear states history? If you do, then you will not be able to " +
                                "undo or redo it.", "Clear Undo/Redo History", MessageBoxButtons.YesNo,
        ...
        public void UpdateCollisionEditorStates(int Steps, int CurrentIndex)
        {
            ...
            if (Steps < 0)
            {
                ...
                for (int step = 0; step < StepsAbs; ++step)
                {
                    ...
                    CollisionState UndoSave = parentEditor.CreateUndoState(UndoIndex);
        ...
        private string ActionText(int Steps)
        {
            return Steps > 1 ? "Actions" : "Action";
        }
        ...
        public class UndoRedoUIListBox : ListBox
        {
            ...
            {
                get => _CurrentUndoRedoIndex;
                set
                {
                    _CurrentUndoRedoIndex = value;
                    Invalidate();
                }
            }
            ...
            protected override void OnDrawItem(DrawItemEventArgs e)
            {
                ...
                if (_CurrentUndoRedoIndex >= 0)
                {
                    ...
                    Color rectangleColor = ((StateItem) Items[index]).isRedo ? Color.SkyBlue : BackColor;
                    ...
                    {
                        if (ListBoxMouseLocation.Value.X > bounds.X &&
                            ListBoxMouseLocation.Value.X < bounds.X + bounds.Width &&
                            ListBoxMouseLocation.Value.Y > bounds.Y &&
                            ListBoxMouseLocation.Value.Y < bounds.Y + bounds.Height)
                        {
                    ...
                    {
                        if (index > _CurrentUndoRedoIndex && index <= _CurrentHoveredIndex ||
                            index >= _CurrentHoveredIndex && index < _CurrentUndoRedoIndex)
                        {
